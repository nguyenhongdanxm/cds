...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    elseif($action==='add'||$action==='update'){
        ...
        $payload=['group'=>$_POST['group']??'','teacher_id'=>$teacherId,'teacher_name'=>$teacherName,'weekdays'=>$_POST['weekdays']??[],'start_date'=>$_POST['start_date']??'','end_date'=>$_POST['end_date']??'','note'=>$_POST['note']??'','created_by'=>$user['name']??'','updated_by'=>$user['name']??''];
        [$ok,$message]=$action==='update'?cds_operation_update_assignment(trim((string)($_POST['id']??'')),$payload):cds_operation_add_assignment($payload);
...
$editing=null;$editId=trim((string)($_GET['edit']??''));if($editId!=='')foreach($rows as$row)if((string)($row['id']??'')===$editId){$editing=$row;break;}
$formGroup=(string)($editing['group']??'management');$formTeacher=(string)($editing['teacher_id']??'');$formDays=array_map('intval',(array)($editing['weekdays']??[]));$formStart=(string)($editing['start_date']??$defaultStart);$formEnd=(string)($editing['end_date']??$defaultEnd);$formNote=(string)($editing['note']??'');
...
<section class="card mb-4"><div class="card-body p-4"><h5 class="section-title"><i class="bi <?=$editing?'bi-pencil-square':'bi-calendar-week'?>"></i> <?=$editing?'Sửa lịch hiển thị':'Gán nhanh theo thứ trong tuần'?></h5><form method="post" class="row g-3"><input type="hidden" name="action" value="<?=$editing?'update':'add'?>"><?php if($editing):?><input type="hidden" name="id" value="<?=e($editing['id']??'')?>"><?php endif;?>
<div class="col-md-4"><label class="form-label fw-semibold">Nhóm hiển thị</label><select class="form-select" name="group" required><?php foreach($groups as$key=>$label):?><option value="<?=e($key)?>" <?=$formGroup===$key?'selected':''?>><?=e($label)?></option><?php endforeach;?></select></div>
<div class="col-md-4"><label class="form-label fw-semibold">Cán bộ/giáo viên</label><select class="form-select" name="teacher_id" required><option value="">Chọn người</option><?php foreach($teachers as$teacher):?><option value="<?=e($teacher['id']??'')?>" <?=$formTeacher===(string)($teacher['id']??'')?'selected':''?>><?=e($teacher['name']??'')?></option><?php endforeach;?></select></div>
<div class="col-md-4"><label class="form-label fw-semibold">Ghi chú</label><input class="form-control" name="note" value="<?=e($formNote)?>" placeholder="Ví dụ: QLNT (không cần nhập ngoặc)"></div>
<div class="col-12"><label class="form-label fw-semibold">Thứ áp dụng</label><div class="weekday-list"><?php foreach($dayLabels as$day=>$label):?><label><input class="form-check-input me-1" type="checkbox" name="weekdays[]" value="<?=$day?>" <?=in_array($day,$formDays,true)?'checked':''?>> <?=e($label)?></label><?php endforeach;?></div></div>
<div class="col-md-4"><label class="form-label fw-semibold">Từ ngày</label><input class="form-control" type="date" name="start_date" id="opsStart" value="<?=e($formStart)?>" required></div><div class="col-md-4"><label class="form-label fw-semibold">Đến ngày</label><input class="form-control" type="date" name="end_date" id="opsEnd" value="<?=e($formEnd)?>" required></div>
...
<div class="col-12 d-flex align-items-center gap-3 flex-wrap"><button class="btn btn-primary px-4"><i class="bi <?=$editing?'bi-check-lg':'bi-plus-lg'?>"></i> <?=$editing?'Lưu thay đổi':'Thêm lịch hiển thị'?></button><?php if($editing):?><a class="btn btn-outline-secondary" href="admin_operation_settings.php">Hủy sửa</a><?php else:?><span class="hint"><i class="bi bi-info-circle"></i> Có thể tạo nhiều người trong cùng một nhóm bằng cách thêm từng lịch.</span><?php endif;?></div></form></div></section>
