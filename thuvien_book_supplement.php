...
$book['intake_history']=$history;$book['quantity']=$oldQty+$qty;$book['last_supplement_date']=$date;$book['updated_at']=date('c');if($source!=='')$book['source']=$source;if($note!=='')$book['note']=trim((string)($book['note']??'').($book['note']??''!==''?"\n":'').'Bổ sung '.$date.': '.$note);$data['books'][$index]=$book;save_json($file,$data);
