...
function cm_progress_auto_metrics(array $assignments, array $year, ?array $week): array {
    ...
    foreach ($assignments as $key => $assignment) {
        ...
        $result[$key] = [
            ...
            'actual_period' => $actualPpct,
            'ppct_period' => $planned,
            'ppct_total' => $ppctMax,
            'ppct_missing' => $ppctMax < 1,
        ];
    }
    return $result;
}
    ...
    if ($action === 'progress_save') {
        ...
        foreach ($rows as $assignmentKey => $values) {
            ...
            $payload = [
                ...
                'standard_weekly' => cm_progress_num($progressAutoByAssignment[$assignmentKey]['standard_weekly'] ?? $assignment['periods'] ?? 0),
                'scheduled_period' => cm_progress_num($progressAutoByAssignment[$assignmentKey]['scheduled_period'] ?? 0),
                'actual_period' => cm_progress_num($progressAutoByAssignment[$assignmentKey]['actual_period'] ?? 0),
                'ppct_period' => cm_progress_num($progressAutoByAssignment[$assignmentKey]['ppct_period'] ?? 0),
...
.progress-table{min-width:1120px;table-layout:fixed}
.progress-table th{font-size:.72rem;line-height:1.15;vertical-align:middle;text-align:center;white-space:normal;padding:.45rem .25rem;width:64px}
.progress-table th:first-child{width:150px}.progress-table th:nth-child(2){width:52px}.progress-table th:nth-child(3){width:100px}
.progress-table td{vertical-align:middle;padding:.35rem .25rem;text-align:center;font-size:.84rem}
.progress-table td:first-child,.progress-table td:nth-child(3){text-align:left}
.progress-table input{width:58px;min-width:58px;max-width:58px;margin:auto;text-align:center;padding-left:.2rem;padding-right:.2rem;font-weight:700}
.progress-auto{background:#f4f7fb!important;color:#29445f;font-weight:700;white-space:nowrap}
...
<div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
  <div>
    ...
    <div class="text-muted">Năm học <?= e($progressYear['label'] ?? '') ?> · Tự động đối chiếu PCCM, TKB, PPCT và Sổ đầu bài đã ký</div>
  </div>
...
<form method="get" class="progress-toolbar card card-body">
  ...
  <div><label class="form-label fw-semibold">Tiến độ</label>
    <select class="form-select" name="status" onchange="this.form.submit()">
      ...
      <option value="missing" <?= $progressStatusFilter==='missing'?'selected':'' ?>>Thiếu PPCT</option>
    </select>
  </div>
  ...
  <div><label class="form-label fw-semibold">Lớp</label>
    <select class="form-select" name="class" onchange="this.form.submit()">
      <option value="">Tất cả lớp</option>
      <?php foreach ($progressFilterClasses as $className): ?><option value="<?= e($className) ?>" <?= $className===$progressClassFilter?'selected':'' ?>><?= e($className) ?></option><?php endforeach; ?>
    </select>
  </div>
  <div><label class="form-label fw-semibold">Giáo viên</label>
  ...
  <?php if ($progressStatusFilter!=='all' || $progressSubjectFilter!=='' || $progressClassFilter!=='' || $progressTeacherFilter!==''): ?>
...
<form method="post">
  ...
  <div class="table-responsive"><table class="table table-bordered table-sm mb-0 progress-table">
    <thead><tr>
      ...
      <th>Tiết TKB<br>đến tuần <?= $progressWeekNumber ?></th>
      <th>Tiết PPCT<br>đã ký</th>
      <th>Tiết KT<br>giữa HK I</th><th>Tiết KT<br>cuối HK I</th>
      <th>Tiết KT<br>giữa HK II</th><th>Tiết KT<br>cuối HK II</th>
      <th>Nhanh/<br>chậm</th>
    ...
    <?php foreach ($progressAssignments as $assignment):
      $key=cm_progress_assignment_key($assignment);$row=$progressMilestonesByAssignment[$key]??[];
      $auto=$progressAutoByAssignment[$key]??[];
      $std=$auto['standard_weekly']??($assignment['periods']??0);$scheduled=$auto['scheduled_period']??0;
      $actual=$auto['actual_period']??0;$ppct=$auto['ppct_period']??0;
    ...
    <tr data-progress-row data-actual="<?= e((string)$actual) ?>">
      ...
      <td class="progress-auto" title="Tự động lấy từ phân công chuyên môn"><?= e((string)$std) ?></td>
      <td class="progress-auto" title="Tự động đếm từ thời khóa biểu"><?= e((string)$scheduled) ?></td>
      <td class="progress-auto" title="Tiết PPCT cao nhất trong Sổ đầu bài đã ký"><?= e((string)$actual) ?></td>
      <?php foreach (['mid_hk1'=>$m1,'final_hk1'=>$f1,'mid_hk2'=>$m2,'final_hk2'=>$f2] as $field=>$value): ?>
      <td><input class="form-control form-control-sm" type="number" min="0" max="999" step="1" inputmode="numeric" name="rows[<?= e($key) ?>][<?= e($field) ?>]" value="<?= e((string)$value) ?>" data-field="<?= e($field) ?>"></td>
      <?php endforeach; ?>
  ...
  <div class="card-body d-flex justify-content-between align-items-center flex-wrap gap-2"><span class="small text-muted"><i class="bi bi-lock-fill"></i> Các cột nền xám được hệ thống tính tự động; giáo viên chỉ nhập 4 mốc kiểm tra.</span><button class="btn btn-primary px-4"><i class="bi bi-floppy"></i> Lưu mốc kiểm tra</button></div></div>
</form>
<script>
document.querySelectorAll('[data-progress-row]').forEach(function(row){
  ...
  function show(name,value){
    var cell=row.querySelector('[data-result="'+name+'"]');
    cell.textContent=Math.max(0,Math.round(value*10)/10);
  }
  function calc(){['mid_hk1','final_hk1','mid_hk2','final_hk2'].forEach(function(field){show(field,n(field)-actual)})}
...
<?php else:
  ...
  foreach($progressAssignmentMap as $key=>$assignment){
    ...
    if($progressClassFilter!=='' && ($assignment['class']??'')!==$progressClassFilter)continue;
    if($progressTeacherFilter!=='' && ($assignment['teacher']??'')!==$progressTeacherFilter)continue;
    $stored=$progressByAssignment[$key]??($progressPreviousByAssignment[$key]??[]);
    $auto=$progressAutoByAssignment[$key]??[];
    $record=array_merge($stored,$auto);
    $diff=!empty($auto['ppct_missing']) ? null : (float)($auto['actual_period']??0)-(float)($auto['ppct_period']??0);
...
<div class="progress-summary">
  ...
  <div class="card"><div class="card-body"><strong class="text-secondary"><?= $missing ?></strong>Thiếu PPCT</div></div>
</div>
...
<table class="table table-hover align-middle mb-0"><thead><tr><th>Giáo viên</th><th>Lớp</th><th>Môn</th><th>PPCT đã ký</th><th>Chuẩn theo TKB</th><th>Nhanh/chậm</th><th>Cảnh báo</th><th>Cập nhật</th></tr></thead><tbody>
...
<?php if($d===null): ?><td colspan="5" class="text-muted">Chưa có PPCT phù hợp để đối chiếu tự động.</td>
