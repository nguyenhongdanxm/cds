...
function cm_observation_rating($score): string {
    return cm_observation_form_rating($score);
}
...
$canSeeRecord = function(array $record) use ($isAdmin,$isLeader,$teacherName,$teacherGroup): bool {
    ...
    foreach(cm_observation_observers($record) as $observer)if(cm_observation_norm($observer)===cm_observation_norm($teacherName))return true;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'observation_review') {
        ...
        if ($invalidObserver) {
            flash('Danh sách người dự chưa hợp lệ.', 'danger'); header('Location: '.BASE_URL.'dugio.php'); exit;
        }
        $updated = false;
        foreach ($records as &$record) {
            if (($record['id'] ?? '') !== $id || !$canSeeRecord($record)) continue;
            $record['observer']=$observers[0]; $record['observers']=$observers; $record['observer_ids']=array_values(array_filter(array_map('cm_observation_teacher_id',$observers))); $record['assignees']=$observers; cm_observation_form_recalculate($record); $record['assigned_by']=$teacherName; $record['assigned_at']=date('c'); $record['updated_at']=date('c'); $updated=true; break;
        }
        unset($record);
        if (!$updated) { http_response_code(403); exit('Không tìm thấy tiết dự giờ trong phạm vi quản lý.'); }
        save_json($dataFile, array_values($records)); flash('Đã lưu phân công người dự. Điểm sẽ tự cập nhật từ các phiếu đánh giá.'); header('Location: '.BASE_URL.'dugio.php'); exit;
    }
