...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'observation_review') {
        ...
        $id = trim((string)($_POST['id'] ?? '')); $observer = trim((string)($_POST['observer'] ?? '')); $scoreRaw = trim((string)($_POST['score'] ?? ''));
        if ($observer === '' || !in_array($observer, $teamTeachers, true) || $scoreRaw === '' || !is_numeric($scoreRaw) || (float)$scoreRaw < 0 || (float)$scoreRaw > 20) {
        ...
        foreach ($records as &$record) {
            ...
            $score = round((float)$scoreRaw, 2); $record['observer']=$observer; $record['assignees']=[$observer]; $record['score']=$score; $record['rating']=cm_observation_rating($score); $record['reviewed_by']=$teacherName; $record['reviewed_at']=date('c'); $record['updated_at']=date('c'); $updated=true; break;
        }
        unset($record);
...
?>
<style>
.obs-toolbar{display:flex;align-items:center;justify-content:space-between;gap:1rem;margin-bottom:1rem}.obs-form-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:.85rem}.obs-form-grid .wide{grid-column:span 2}.obs-table{min-width:1450px}.obs-table th,.obs-table td{vertical-align:middle}.obs-rating{display:inline-flex;padding:.28rem .55rem;border-radius:999px;font-size:.75rem;font-weight:800}.obs-rating.good{background:#d1e7dd;color:#0f5132}.obs-rating.fair{background:#cff4fc;color:#055160}.obs-rating.pass{background:#fff3cd;color:#664d03}.obs-rating.fail{background:#f8d7da;color:#842029}.obs-review{display:grid;grid-template-columns:minmax(150px,1fr) 80px auto;gap:.35rem;min-width:330px}.obs-kpis{display:grid;grid-template-columns:repeat(6,1fr);gap:.75rem;margin-bottom:1rem}.obs-kpis>div{padding:1rem;border-radius:12px;background:#fff;box-shadow:0 2px 12px rgba(0,0,0,.07);text-align:center}.obs-kpis strong,.obs-kpis span{display:block}.obs-kpis strong{font-size:1.65rem;color:var(--primary)}.obs-kpis span{font-size:.78rem;color:#64748b}.obs-summary{min-width:850px}@media(max-width:767px){.obs-toolbar{align-items:flex-start;flex-direction:column}.obs-form-grid{grid-template-columns:1fr 1fr}.obs-form-grid .wide{grid-column:1/-1}.obs-kpis{grid-template-columns:repeat(2,1fr)}.obs-kpis>div:first-child{grid-column:1/-1}}
.obs-table{table-layout:fixed}.obs-register-table{min-width:1485px}.obs-detail-table{min-width:1425px}.obs-table th{text-align:center;vertical-align:middle}.obs-table td{overflow-wrap:anywhere}.obs-register-table td:nth-child(1),.obs-register-table td:nth-child(4),.obs-register-table td:nth-child(5),.obs-register-table td:nth-child(6),.obs-register-table td:nth-child(7),.obs-register-table td:nth-child(8),.obs-register-table td:nth-child(11),.obs-register-table td:nth-child(12),.obs-register-table td:nth-child(13),.obs-detail-table td:nth-child(1),.obs-detail-table td:nth-child(5),.obs-detail-table td:nth-child(6),.obs-detail-table td:nth-child(7),.obs-detail-table td:nth-child(8),.obs-detail-table td:nth-child(9),.obs-detail-table td:nth-child(12),.obs-detail-table td:nth-child(13){text-align:center}.obs-summary th,.obs-summary td{text-align:center;vertical-align:middle}.obs-summary th:nth-child(2),.obs-summary th:nth-child(3),.obs-summary td:nth-child(2),.obs-summary td:nth-child(3){text-align:left}.obs-filter-note{font-size:.78rem;color:#64748b}.obs-filter-actions{display:flex;gap:.5rem;align-items:center}@media(max-width:767px){.obs-filter-actions{width:100%}.obs-filter-actions .btn{flex:1}}
</style>
...
<?php if(($isAdmin||$isLeader)&&$canSeeRecord($record)):?><td colspan="3"><form method="post" class="obs-review"><input type="hidden" name="action" value="observation_review"><input type="hidden" name="csrf" value="<?=e($csrf)?>"><input type="hidden" name="id" value="<?=e($record['id']??'')?>"><select class="form-select form-select-sm" name="observer" required><option value="">Chọn người dự</option><?php foreach($teamTeachers as $name):?><option value="<?=e($name)?>" <?=$name===($record['observer']??'')?'selected':''?>><?=e($name)?></option><?php endforeach;?></select><input class="form-control form-control-sm obs-score" type="number" name="score" min="0" max="20" step=".01" value="<?=e((string)($record['score']??''))?>" placeholder="Điểm" required><button class="btn btn-sm btn-success" title="Lưu đánh giá"><i class="bi bi-check-lg"></i></button><small class="text-muted obs-live-rating" style="grid-column:1/-1"><?=$rating?e($rating):'Xếp loại tự động theo điểm'?></small></form></td>
<?php else:?><td><?=e($record['observer']??'—')?></td><td><?=($record['score']??'')!==''?e((string)$record['score']):'—'?></td><td><?=$rating?'<span class="obs-rating '.$ratingClass.'">'.e($rating).'</span>':'—'?></td><?php endif;?>
...
<div class="card"><div class="card-header">Bảng kê chi tiết</div><div class="table-responsive"><table class="table table-bordered table-sm mb-0 obs-table obs-detail-table"><colgroup><col style="width:52px"><col style="width:185px"><col style="width:150px"><col style="width:115px"><col style="width:72px"><col style="width:82px"><col style="width:105px"><col style="width:76px"><col style="width:72px"><col style="width:245px"><col style="width:180px"><col style="width:72px"><col style="width:95px"></colgroup><thead><tr><th>STT</th><th>Họ tên giáo viên</th><th>Tổ</th><th>Môn</th><th>Lớp</th><th>Tuần</th><th>Ngày</th><th>Tiết PPCT</th><th>Tiết TKB</th><th>Tên bài dạy</th><th>Người dự</th><th>Điểm</th><th>Xếp loại</th></tr></thead><tbody><?php if(!$statsRecords):?><tr><td colspan="13" class="text-center text-muted py-4">Chưa có dữ liệu.</td></tr><?php else:foreach($statsRecords as $i=>$record):?><tr><td><?=$i+1?></td><td><?=e($record['teacher']??'')?></td><td><?=e($record['teacher_group']??'')?></td><td><?=e($record['subject']??'')?></td><td><?=e($record['class']??'')?></td><td>Tuần <?=(int)($record['week_number']??0)?></td><td><?=!empty($record['date'])?date('d/m/Y',strtotime($record['date'])):'—'?></td><td><?=(int)($record['ppct_period']??0)?></td><td><?=(int)($record['timetable_period']??0)?></td><td><?=e($record['lesson_title']??'')?></td><td><?=e($record['observer']??'')?></td><td><?=e((string)($record['score']??''))?></td><td><?=e($record['rating']??'')?></td></tr><?php endforeach;endif;?></tbody></table></div></div>
<?php endif;?>
