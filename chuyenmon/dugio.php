...
$statsRecords = array_values(array_filter($visibleRecords, function($record) use($filterGroup,$filterSubject){
    return ($filterGroup==='' || ($record['teacher_group']??'')===$filterGroup) && ($filterSubject==='' || ($record['subject']??'')===$filterSubject);
}));
...
<div class="card"><div class="card-header d-flex justify-content-between"><span>Danh sách dự giờ</span><span><?=count($visibleRecords)?> tiết</span></div><div class="table-responsive"><table class="table table-bordered table-hover mb-0 obs-table"><thead><tr><th>STT</th><th>Họ tên giáo viên</th><th>Môn</th><th>Lớp</th><th>Tuần</th><th>Ngày</th><th>Tiết PPCT</th><th>Tiết TKB</th><th>Tên bài dạy</th><th>Người dự</th><th>Điểm</th><th>Xếp loại</th><th>Thao tác</th></tr></thead><tbody>
...
<?php else:?>
<form method="get" class="card mb-3"><input type="hidden" name="view" value="stats"><div class="card-body row g-2 align-items-end"><div class="col-md-5"><label class="form-label fw-semibold">Tổ chuyên môn</label><select class="form-select" name="group" <?=$isLeader&&!$isAdmin?'disabled':''?>><option value="">Tất cả tổ</option><?php foreach($filterGroups as $group):?><option value="<?=e($group)?>" <?=$group===$filterGroup?'selected':''?>><?=e($group)?></option><?php endforeach;?></select><?php if($isLeader&&!$isAdmin):?><input type="hidden" name="group" value="<?=e($teacherGroup)?>"><?php endif;?></div><div class="col-md-5"><label class="form-label fw-semibold">Môn</label><select class="form-select" name="subject"><option value="">Tất cả môn</option><?php foreach($filterSubjects as $subject):?><option value="<?=e($subject)?>" <?=$subject===$filterSubject?'selected':''?>><?=e($subject)?></option><?php endforeach;?></select></div><div class="col-md-2"><button class="btn btn-primary w-100"><i class="bi bi-funnel"></i> Lọc</button></div></div></form>
