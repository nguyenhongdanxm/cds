...
function lb_ppct_import_v2(array $file,array $input=[]): array {
 ...
 $rows=lb_curriculum();$index=[];foreach($rows as$i=>$r){$index[lb_norm((string)($r['subject']??'')).'|'.lb_ppct_grade_v2((string)($r['grade']??'')).'|'.(int)($r['period']??0).'|'.(string)($r['scope_type']??'school').'|'.lb_norm((string)($r['scope_target']??''))]=$i;}$added=0;$updated=0;
