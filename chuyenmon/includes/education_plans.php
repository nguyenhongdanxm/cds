...
(function(){var f=document.getElementById('educationPlanForm'),file=document.getElementById('educationFile'),box=document.getElementById('educationUploadProgress'),bar=document.getElementById('educationUploadBar'),pct=document.getElementById('educationUploadPercent'),st=document.getElementById('educationUploadStatus'),dt=document.getElementById('educationUploadDetail'),sb=document.getElementById('educationSubmitButton'),cb=document.getElementById('educationCancelButton'),endpoint='<?=e(BASE_URL.'education_upload.php')?>';function p(v){v=Math.max(0,Math.min(100,Math.round(v)));bar.style.width=v+'%';pct.textContent=v+'%'}function busy(v){sb.disabled=v;cb.disabled=v}function fail(m){busy(false);bar.className='progress-bar bg-danger';st.innerHTML='<i class="bi bi-x-circle-fill text-danger me-2"></i>Tải lên chưa thành công';dt.textContent=m||'Không kết nối được máy chủ.'}function done(r){p(100);bar.className='progress-bar bg-success';st.innerHTML='<i class="bi bi-check-circle-fill text-success me-2"></i>Tải lên thành công';dt.textContent=r.message||'Đã lưu PDF vào Google Drive.';setTimeout(function(){location.href=r.redirect||location.href},500)}async function postJson(url,data){var res=await fetch(url,{method:'POST',body:data,credentials:'same-origin'}),text=await res.text(),json;try{json=JSON.parse(text)}catch(e){throw new Error('Máy chủ trả về dữ liệu không hợp lệ'+(text.trim()?': '+text.replace(/<[^>]+>/g,' ').trim().slice(0,180):'.'))}if(!res.ok||!json.ok)throw new Error(json.message||('Lỗi HTTP '+res.status));return json}function directPut(url,pdf){return new Promise(function(resolve,reject){var x=new XMLHttpRequest();x.open('PUT',url,true);x.timeout=900000;x.setRequestHeader('Content-Type','application/pdf');x.upload.onprogress=function(a){if(a.lengthComputable)p(8+a.loaded/a.total*84)};x.onload=function(){var r={};try{r=JSON.parse(x.responseText||'{}')}catch(e){}if(x.status>=200&&x.status<300&&r.id)resolve(r);else reject(new Error('Google Drive chưa xác nhận tệp (mã '+x.status+').') )};x.onerror=function(){reject(new Error('Trình duyệt không kết nối được phiên tải trực tiếp Google Drive.'))};x.ontimeout=function(){reject(new Error('Tải PDF lên Google Drive quá thời gian.'))};x.send(pdf)})}f.addEventListener('submit',async function(e){e.preventDefault();if(!f.reportValidity())return;box.hidden=false;bar.className='progress-bar progress-bar-striped progress-bar-animated';p(1);busy(true);try{var pdf=file.files&&file.files[0];if(!pdf){var normal=new FormData(f);normal.delete('file');done(await postJson(f.getAttribute('action')||location.href,normal));return}if(pdf.size>25*1024*1024)throw new Error('Tệp PDF vượt quá 25 MB.');dt.textContent='Đang tạo phiên tải trực tiếp lên Google Drive…';var init=new FormData();init.set('csrf',f.elements.csrf.value);init.set('action','init');init.set('id',f.elements.id.value);init.set('appendix',f.elements.appendix.value);init.set('subject',f.elements.subject.value);init.set('grade',f.elements.grade.value);init.set('file_name',pdf.name);init.set('file_size',String(pdf.size));var session=await postJson(endpoint,init);p(7);dt.textContent='Đang tải PDF trực tiếp lên Google Drive, không qua giới hạn cPanel…';var drive=await directPut(session.upload_url,pdf);p(94);dt.textContent='Đang xác minh và cập nhật kế hoạch…';var finalData=new FormData();finalData.set('csrf',f.elements.csrf.value);finalData.set('action','finalize');finalData.set('subject',f.elements.subject.value);finalData.set('grade',f.elements.grade.value);finalData.set('appendix',f.elements.appendix.value);finalData.set('upload_token',session.upload_token);finalData.set('drive_file_id',drive.id);done(await postJson(endpoint,finalData))}catch(error){fail(error&&error.message?error.message:'Không tải được PDF.')}})})();
</script>
<?php if($educationIsAdmin||$educationIsLeader): ?>
<script>
(function(){
