...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete_plan') {
        ...
        cm_education_save($educationDataFile, $educationRows); flash('Đã xóa kế hoạch.', 'warning'); cm_education_redirect($appendix);
    }

    if ($action === 'approve_plan') {
        if (!$educationIsAdmin && !$educationIsLeader) { http_response_code(403); exit('Chỉ TTCM hoặc quản trị được duyệt.'); }
        $id = trim((string)($_POST['id'] ?? '')); $approved = false;
        foreach ($educationRows as &$row) {
            if (($row['id'] ?? '') !== $id) continue;
            ...
            $appendix = (string)($row['appendix'] ?? $appendix);
            if (empty($row['approved_at'])) { $row['approved_at'] = date('c'); $row['approved_by'] = $educationTeacher ?: (string)($educationUser['name'] ?? 'Quản trị'); $row['updated_at'] = date('c'); }
            $approved = true; break;
        }
        unset($row);
        if (!$approved) { http_response_code(404); exit('Không tìm thấy kế hoạch.'); }
        cm_education_save($educationDataFile, $educationRows); flash('Đã duyệt kế hoạch. Giáo viên không thể sửa hoặc xóa sau thời điểm này.'); cm_education_redirect($appendix);
    }
}
...
// appendix dùng cho tab nhanh và giá trị mặc định khi nhập mới; appendix_filter là bộ lọc danh sách độc lập.
...
// Khi bấm tab Phụ lục I/II/III thì tự lọc đúng phụ lục; khi chọn “Tất cả” ở bộ lọc sẽ bỏ giới hạn này.
...
$educationList = array_values(array_filter($educationVisibleRows, function($row) use ($educationFilterAppendix,$educationSearch,$educationFilterGroup,$educationFilterSubject,$educationFilterGrade) {
...
?>
<style>
.education-toolbar{display:flex;align-items:center;justify-content:space-between;gap:1rem;margin-bottom:1rem}
.education-tabs .nav-link{min-width:130px;text-align:center}
.education-filter{display:grid;grid-template-columns:2fr repeat(4,minmax(125px,1fr)) auto;gap:.65rem;align-items:end}
.education-table{min-width:1180px}.education-table td{vertical-align:middle}
.education-appendix{display:inline-flex;align-items:center;justify-content:center;min-width:76px;padding:.28rem .55rem;border-radius:999px;background:#e8f1fb;color:#164f7b;font-size:.78rem;font-weight:800}
.education-status{display:inline-flex;align-items:center;gap:.3rem;padding:.28rem .58rem;border-radius:999px;font-size:.78rem;font-weight:700}
.education-status.pending{background:#fff3cd;color:#664d03}.education-status.approved{background:#d1e7dd;color:#0f5132}
.education-meta{font-size:.78rem;line-height:1.45;color:#64748b}
.education-form-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:.85rem}.education-form-grid .wide{grid-column:1/-1}
@media(max-width:1100px){.education-filter{grid-template-columns:1fr 1fr 1fr}.education-filter .search{grid-column:1/-1}}
...
@media(max-width:575px){.education-filter{grid-template-columns:1fr}.education-filter .search{grid-column:auto}}
</style>
...
<ul class="nav nav-pills education-tabs gap-2 mb-3">
  <li class="nav-item"><a class="nav-link <?= $educationFilterAppendix === '' ? 'active' : '' ?>" href="<?= BASE_URL ?>kehoach.php?tab=vanban&appendix=<?= e($educationAppendix) ?>&appendix_filter=">Tất cả phụ lục</a></li>
  <?php foreach (['I','II','III'] as $appendix): ?><li class="nav-item"><a class="nav-link <?= $educationFilterAppendix === $appendix ? 'active' : '' ?>" href="<?= BASE_URL ?>kehoach.php?tab=vanban&appendix=<?= $appendix ?>&appendix_filter=<?= $appendix ?>">Phụ lục <?= $appendix ?></a></li><?php endforeach; ?>
</ul>
...
<form method="get" class="education-filter">
  ...
  <div class="search"><label class="form-label small fw-semibold">Tìm kiếm</label><input class="form-control" name="q" value="<?= e($educationSearch) ?>" placeholder="Tên giáo viên, tổ, môn, khối hoặc phụ lục"></div>
  <div><label class="form-label small fw-semibold">Phụ lục</label><select class="form-select" name="appendix_filter"><option value="">Tất cả phụ lục</option><?php foreach(['I','II','III'] as $value): ?><option value="<?= $value ?>" <?= $educationFilterAppendix===$value?'selected':'' ?>>Phụ lục <?= $value ?></option><?php endforeach; ?></select></div>
  ...
  <div class="d-flex gap-2"><button class="btn btn-primary" title="Lọc"><i class="bi bi-funnel"></i></button><a class="btn btn-outline-secondary" href="<?= BASE_URL ?>kehoach.php?tab=vanban&appendix=<?= e($educationAppendix) ?>&appendix_filter=" title="Bỏ lọc"><i class="bi bi-arrow-counterclockwise"></i></a></div>
</form>
</div></div>

<div class="card"><div class="card-header d-flex justify-content-between"><span><?= $educationFilterAppendix!=='' ? 'Phụ lục '.e($educationFilterAppendix) : 'Tất cả phụ lục' ?></span><span><?= count($educationList) ?> văn bản</span></div><div class="table-responsive">
<table class="table table-hover mb-0 education-table"><thead><tr><th>STT</th><th>Giáo viên</th><th>Phụ lục</th><th>Tổ</th><th>Môn</th><th>Khối</th><th>Thời gian</th><th>Trạng thái</th><th>Thao tác</th></tr></thead><tbody>
<?php if(!$educationList): ?><tr><td colspan="9" class="text-center text-muted py-4">Không có kế hoạch phù hợp bộ lọc.</td></tr>
<?php else: foreach($educationList as $index=>$row): $approved=!empty($row['approved_at']); $owner=cm_education_norm($row['teacher']??'')===cm_education_norm($educationTeacher); $rowAppendix=(string)($row['appendix']??''); ?>
<tr>
  <td class="text-center"><?= $index+1 ?></td>
  <td><strong><?= e($row['teacher']??'') ?></strong></td>
  <td class="text-center"><span class="education-appendix">Phụ lục <?= e($rowAppendix ?: '—') ?></span></td>
  <td><?= e($row['teacher_group']??'') ?></td><td><?= e($row['subject']??'') ?></td><td class="text-center"><?= e($row['grade']??'') ?></td>
  ...
  <td class="text-nowrap"><a class="btn btn-sm btn-outline-success" target="_blank" rel="noopener" href="<?= e(BASE_URL.'education_file.php?id='.urlencode((string)($row['id']??''))) ?>" title="Xem PDF"><i class="bi bi-eye"></i></a>
...
<div class="education-form-grid">
  <div><label class="form-label fw-semibold">Tên giáo viên</label><input class="form-control" value="<?= e($educationTeacher) ?>" readonly></div>
  <div><label class="form-label fw-semibold">Tổ</label><input class="form-control" value="<?= e($educationGroup) ?>" readonly></div>
  <div><label class="form-label fw-semibold">Phụ lục</label><select class="form-select" name="appendix" id="educationAppendix" required><?php foreach(['I','II','III'] as $value): ?><option value="<?= $value ?>" <?= $value===$educationAppendix?'selected':'' ?>>Phụ lục <?= $value ?></option><?php endforeach; ?></select></div>
  ...
  <div class="wide"><label class="form-label fw-semibold">Văn bản PDF ký số</label><input class="form-control" type="file" name="file" id="educationFile" accept="application/pdf,.pdf"><div class="form-text" id="educationFileNote">Chỉ tải tệp PDF đã ký số, tối đa 25 MB.</div></div>
</div>
<div class="mt-3 p-3 border rounded bg-light" id="educationUploadProgress" hidden aria-live="polite"><div class="d-flex justify-content-between align-items-center gap-2 mb-2"><span class="fw-semibold" id="educationUploadStatus"><span class="spinner-border spinner-border-sm me-2"></span>Đang chuẩn bị tải lên…</span><strong id="educationUploadPercent">0%</strong></div><div class="progress"><div class="progress-bar progress-bar-striped progress-bar-animated" id="educationUploadBar" style="width:0%"></div></div><div class="small text-muted mt-2" id="educationUploadDetail">Vui lòng giữ nguyên trang cho đến khi có thông báo thành công.</div></div>
</div>
<div class="modal-footer"><button class="btn btn-outline-secondary" id="educationCancelButton" type="button" data-bs-dismiss="modal">Hủy</button><button class="btn btn-primary" id="educationSubmitButton" type="submit"><i class="bi bi-cloud-arrow-up"></i> Tải lên và lưu</button></div>
</form></div></div></div>
...
function editEducationPlan(button){try{var row=JSON.parse(decodeURIComponent(escape(atob(button.dataset.plan||''))));document.getElementById('educationId').value=row.id||'';document.getElementById('educationAppendix').value=row.appendix||'I';document.getElementById('educationSubject').value=row.subject||'';educationFilterGrades();document.getElementById('educationGrade').value=row.grade||'';document.getElementById('educationFile').required=false;document.getElementById('educationFileNote').textContent='Để trống nếu giữ nguyên PDF hiện tại; chọn tệp mới để thay thế.';document.getElementById('educationUploadProgress').hidden=true;bootstrap.Modal.getOrCreateInstance(document.getElementById('educationPlanModal')).show()}catch(error){alert('Không đọc được dữ liệu kế hoạch.')}}
(function(){
 ...
 function setProgress(value){value=Math.max(0,Math.min(100,Math.round(value)));bar.style.width=value+'%';percent.textContent=value+'%'}
 ...
 function parse(xhr){try{return JSON.parse(xhr.responseText||'{}')}catch(error){return {ok:false,message:xhr.status===413?'Tệp vượt quá giới hạn tải lên của máy chủ cPanel.':'Máy chủ trả về dữ liệu không hợp lệ (HTTP '+xhr.status+').'}}}
 form.addEventListener('submit',function(event){event.preventDefault();if(!form.reportValidity())return;box.hidden=false;bar.className='progress-bar progress-bar-striped progress-bar-animated';setProgress(0);setBusy(true);status.innerHTML='<span class="spinner-border spinner-border-sm me-2"></span>Đang tải tệp lên máy chủ…';detail.textContent='Vui lòng giữ nguyên trang trong khi tải.';var xhr=new XMLHttpRequest(),data=new FormData(form);xhr.open('POST',form.getAttribute('action')||window.location.href,true);xhr.timeout=600000;xhr.setRequestHeader('X-Requested-With','XMLHttpRequest');xhr.upload.onprogress=function(p){if(!p.lengthComputable)return;var value=p.loaded/p.total*100;setProgress(value);detail.textContent='Đã gửi '+Math.round(p.loaded/1024)+' KB / '+Math.round(p.total/1024)+' KB.';if(value>=99.5){status.innerHTML='<span class="spinner-border spinner-border-sm me-2"></span>Đã tải 100% — đang lưu vào Google Drive…'}};xhr.onload=function(){var result=parse(xhr);if(xhr.status<200||xhr.status>=300||!result.ok){fail(result.message||'Máy chủ không lưu được tệp.');return}setProgress(100);bar.className='progress-bar bg-success';status.innerHTML='<i class="bi bi-check-circle-fill text-success me-2"></i>Tải lên thành công';detail.textContent=result.message||'Đã lưu PDF vào Google Drive.';setTimeout(function(){window.location.href=result.redirect||window.location.href},500)};xhr.onerror=function(){fail('Mất kết nối tới máy chủ cPanel trong khi tải tệp.')};xhr.ontimeout=function(){fail('Máy chủ xử lý quá lâu. Vui lòng kiểm tra lại kết nối Google Drive.')};xhr.send(data)});
})();
</script>
