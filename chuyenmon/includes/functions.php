...
function cds_drive_settings(): array {$raw=is_file(CDS_DRIVE_SETTINGS)?json_decode((string)file_get_contents(CDS_DRIVE_SETTINGS),true):[];$raw=is_array($raw)?$raw:[];$legacy=(array)($raw['folders']??[]);$types=is_array($raw['types']??null)?$raw['types']:[];foreach(['documents'=>'Văn bản','plans'=>'Kế hoạch và báo cáo','photos'=>'Ảnh học sinh'] as $key=>$label){$types[$key]=array_merge(['label'=>$label,'folder_id'=>''],is_array($types[$key]??null)?$types[$key]:[]);if(empty($types[$key]['folder_id'])&&!empty($legacy[$key]))$types[$key]['folder_id']=$legacy[$key];}return array_merge(['enabled'=>false,'oauth'=>[],'types'=>$types],$raw,['types'=>$types]);}
...
function cds_drive_token(array $settings=[]): array {$settings=$settings?:cds_drive_settings();$oauth=(array)($settings['oauth']??[]);if(!empty($oauth['client_id'])&&!empty($oauth['client_secret'])&&!empty($oauth['refresh_token'])){$res=cds_drive_http('https://oauth2.googleapis.com/token','POST',['Content-Type: application/x-www-form-urlencoded'],http_build_query(['client_id'=>$oauth['client_id'],'client_secret'=>$oauth['client_secret'],'refresh_token'=>$oauth['refresh_token'],'grant_type'=>'refresh_token']));$json=json_decode($res['body'],true);return (!$res['ok']||empty($json['access_token']))?['ok'=>false,'message'=>$json['error_description']??'Kết nối Google hết hiệu lực.']:['ok'=>true,'token'=>$json['access_token']];}return ['ok'=>false,'message'=>'Chưa kết nối OAuth Google Drive.'];}
function cds_drive_upload_bytes(string $bytes,string $name,string $mime,string $category): array {$settings=cds_drive_settings();$folder=trim((string)($settings['types'][$category]['folder_id']??''));if(empty($settings['enabled'])||$folder==='')return ['ok'=>false,'disabled'=>true,'message'=>'Google Drive chưa bật hoặc chưa chọn thư mục.'];$token=cds_drive_token($settings);if(empty($token['ok']))return $token;$boundary='cds-'.bin2hex(random_bytes(12));$meta=json_encode(['name'=>$name,'parents'=>[$folder]],JSON_UNESCAPED_UNICODE);$body="--$boundary\r\nContent-Type: application/json; charset=UTF-8\r\n\r\n$meta\r\n--$boundary\r\nContent-Type: $mime\r\n\r\n$bytes\r\n--$boundary--";$res=cds_drive_http('https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id,name','POST',['Authorization: Bearer '.$token['token'],'Content-Type: multipart/related; boundary='.$boundary],$body);$json=json_decode($res['body'],true);if(!$res['ok']||empty($json['id']))return ['ok'=>false,'message'=>$json['error']['message']??'Không tải được file lên Drive.'];$history=is_file(CDS_DRIVE_HISTORY)?json_decode((string)file_get_contents(CDS_DRIVE_HISTORY),true):[];$history=is_array($history)?$history:[];array_unshift($history,['at'=>date('c'),'by'=>$_SESSION['cds_user']['name']??'','action'=>'upload','type'=>$category,'name'=>$name,'file_id'=>$json['id'],'folder_id'=>$folder]);file_put_contents(CDS_DRIVE_HISTORY,json_encode(array_slice($history,0,1000),JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT),LOCK_EX);return ['ok'=>true,'id'=>$json['id'],'path'=>'gdrive:'.$json['id']];}
