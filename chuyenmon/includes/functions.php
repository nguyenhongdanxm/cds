...
function cds_drive_settings(): array {$raw=is_file(CDS_DRIVE_SETTINGS)?json_decode((string)file_get_contents(CDS_DRIVE_SETTINGS),true):[];$raw=is_array($raw)?$raw:[];$legacy=(array)($raw['folders']??[]);$types=is_array($raw['types']??null)?$raw['types']:[];foreach(['documents'=>'Văn bản','plans'=>'Kế hoạch và báo cáo','photos'=>'Ảnh học sinh'] as $key=>$label){$types[$key]=array_merge(['label'=>$label,'folder_id'=>''],is_array($types[$key]??null)?$types[$key]:[]);if(empty($types[$key]['folder_id'])&&!empty($legacy[$key]))$types[$key]['folder_id']=$legacy[$key];}return array_merge(['enabled'=>false,'oauth'=>[],'types'=>$types,'bindings'=>[]],$raw,['types'=>$types,'bindings'=>is_array($raw['bindings']??null)?$raw['bindings']:[]]);}
