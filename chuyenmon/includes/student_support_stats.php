...
$statsView=(string)($_GET['stats_view']??'summary'); if(!in_array($statsView,['summary','groups','overlap'],true)) $statsView='summary';
...
$statGender=static function($value): string {
    $value=trim((string)$value);
    $lower=function_exists('mb_strtolower')?mb_strtolower($value,'UTF-8'):strtolower($value);
    if(in_array($lower,['nam','m','male'],true)) return 'Nam';
    if(in_array($lower,['nữ','nu','f','female'],true)) return 'Nữ';
    return 'Chưa rõ';
};
// Tổng chung đếm học sinh duy nhất dù đăng ký nhiều môn; từng môn đếm riêng.
$examSummary=[];
if(in_array($categoryFilter,['tn','ts'],true)) {
    foreach($examByStudent[$categoryFilter] as $id=>$subjectGroups) {
        $student=$students[$id]; $cl=$student['class'];
        $chosen=$subjectFilter!==''?array_intersect_key($subjectGroups,[$subjectFilter=>true]):$subjectGroups;
        if(!$chosen) continue;
        $examSummary[''][$cl][$id]=true;
        foreach($chosen as $sub=>$_) $examSummary[$sub][$cl][$id]=true;
    }
}
$examSummaryRows=[];
...
foreach(array_merge([''],$summarySubjects) as $sub) {
    ...
    $examSummaryRows[]=['subject'=>$sub,'class'=>'','ids'=>$allIds];
    $scopeClasses=[];
    foreach($examClasses[$categoryFilter] as $classId=>$_) {
        $cl=$classOptions[$classId]??'';
        if($cl!=='' && ($gradeFilter==='' || $statGrade($cl)===$gradeFilter) && ($classFilter==='' || $classFilter===$cl)) $scopeClasses[$cl]=true;
    }
...
if ($detailMode === 'exam_summary' && in_array($categoryFilter,['tn','ts'],true)) {
    $targetSubject=(string)($_GET['detail_subject']??'');
    $targetClass=(string)($_GET['detail_class']??'');
    $targetGender=(string)($_GET['detail_gender']??'');
    if(in_array($targetGender,['','Nam','Nữ','Chưa rõ'],true)) foreach($examSummaryRows as $row) if($row['subject']===$targetSubject && $row['class']===$targetClass) {
        $detailIds=array_keys(array_filter($row['ids'],static function($_,$id) use($targetGender,$students,$statGender) { return $targetGender==='' || $statGender($students[$id]['gender']??'')===$targetGender; },ARRAY_FILTER_USE_BOTH));
