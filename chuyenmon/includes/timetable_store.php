...
function tkb_resolved_slots(array $week):array{$mapping=tkb_mapping();$slots=array_values(array_filter((array)($week['slots']??[]),'is_array'));return array_map(fn($slot)=>tkb_resolve_slot($slot,$mapping),$slots);}
...
function tkb_import_week(string $path,string $schoolWeekKey,array $teachers,array $classes,string $by=''):array{
    ...
    $mapping=tkb_mapping();$teacherAliases=[];$classAliases=[];
    foreach($parsed['slots']as$slot){if(trim((string)$slot['teacher_raw'])!=='')$teacherAliases[$slot['teacher_raw']]=true;$classAliases[$slot['class_raw']]=true;}
...
function tkb_save_mapping(array $teacherMap,array $classMap):bool{$mapping=tkb_mapping();foreach($teacherMap as$raw=>$value){$raw=trim((string)$raw);$value=trim((string)$value);if($raw==='')continue;if($value==='')unset($mapping['teachers'][$raw]);else$mapping['teachers'][$raw]=$value;}foreach($classMap as$raw=>$value){$raw=trim((string)$raw);$value=trim((string)$value);if($raw==='')continue;if($value==='')unset($mapping['classes'][$raw]);else$mapping['classes'][$raw]=$value;}return tkb_save(TKB_MAPPING_FILE,$mapping);}
