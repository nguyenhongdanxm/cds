...
function tkb_resolve_slot(array $slot,array $mapping):array{$rawTeacher=trim((string)($slot['teacher_raw']??$slot['teacher']??''));$rawClass=trim((string)($slot['class_raw']??$slot['class']??''));$slot['teacher']=(string)($mapping['teachers'][$rawTeacher]??$rawTeacher);$slot['class']=(string)($mapping['classes'][$rawClass]??$rawClass);$rawSubject=trim((string)($slot['subject_raw']??$slot['subject']??''));$slot['subject']=tkb_subject_canonical((string)($mapping['subjects'][$rawSubject]??$rawSubject));return$slot;}
function tkb_resolved_slots(array $week):array{$mapping=tkb_mapping();$slots=array_values(array_filter((array)($week['slots']??[]),'is_array'));return array_map(function($slot)use($mapping,$week){$slot=tkb_resolve_slot($slot,$mapping);$slot['subject']=tkb_subject_canonical((string)($slot['subject']??''));foreach((array)($week['rooms']??[]) as $room)if((string)($room['id']??'')===(string)($slot['room_id']??'')){$slot['room_name']=(string)($room['name']??'');break;}return$slot;},$slots);}
function tkb_special_applies_to_class(array $slot,string $class):bool{$scope=(string)($slot['scope']??'school');$value=trim((string)($slot['value']??''));if($scope==='school')return true;if($scope==='class')return tkb_key($value)===tkb_key($class);if($scope==='grade'&&preg_match('/\d+/u',$class,$m))return(ltrim($m[0],'0')?:'0')===(ltrim($value,'0')?:'0');return false;}
function tkb_special_slot_row(array $slot,string $class=''):array{$scope=(string)($slot['scope']??'school');$value=trim((string)($slot['value']??''));$scopeLabel=$scope==='school'?'Toàn trường':($scope==='grade'?'Khối '.$value:'Lớp '.$value);return['day'=>(int)($slot['day']??0),'session'=>(string)($slot['session']??'Sáng'),'period'=>(int)($slot['period']??0),'class'=>$class,'class_raw'=>$class,'subject'=>(string)($slot['label']??'Hoạt động chung'),'teacher'=>'','teacher_raw'=>'','_special'=>true,'_special_id'=>(string)($slot['id']??''),'_special_scope'=>$scopeLabel];}
...
function tkb_excel_col(int $n):string{$s='';while($n>0){$n--;$s=chr(65+($n%26)).$s;$n=intdiv($n,26);}return$s;}
function tkb_template_rows(?array $week,array $classes):array{
    $headers=['Lớp','Buổi','Tiết'];for($d=2;$d<=7;$d++){$headers[]='Thứ '.$d.' - Môn';$headers[]='Thứ '.$d.' - Giáo viên';}$rows=[$headers];
    $grid=[];$shapes=[];
    if($week)foreach(tkb_resolved_slots($week)as$slot){$class=trim((string)($slot['class']??$slot['class_raw']??''));$session=mb_stripos((string)($slot['session']??''),'chiều',0,'UTF-8')!==false?'Chiều':'Sáng';$period=max(1,(int)($slot['period']??1));$day=(int)($slot['day']??0);if($class===''||$day<2||$day>7)continue;$shapes[$class][$session][$period]=true;$grid[$class][$session][$period][$day]=[(string)($slot['subject']??''),(string)($slot['teacher']??$slot['teacher_raw']??'')];}
    if(!$week)foreach($classes as$class)foreach(['Sáng','Chiều']as$session)for($period=1;$period<=5;$period++)$shapes[(string)$class][$session][$period]=true;
    uksort($shapes,'strnatcasecmp');foreach($shapes as$class=>$sessions)foreach(['Sáng','Chiều']as$session)if(!empty($sessions[$session])){$periods=array_keys($sessions[$session]);sort($periods,SORT_NUMERIC);foreach($periods as$period){$row=[$class,$session,(string)$period];for($d=2;$d<=7;$d++){$row[]=(string)($grid[$class][$session][$period][$d][0]??'');$row[]=(string)($grid[$class][$session][$period][$d][1]??'');}$rows[]=$row;}}
    return$rows;
}
function tkb_xlsx_cell(int $col,int $row,string $value):string{$ref=tkb_excel_col($col).$row;$value=htmlspecialchars($value,ENT_XML1|ENT_QUOTES,'UTF-8');return'<c r="'.$ref.'" t="inlineStr"><is><t xml:space="preserve">'.$value.'</t></is></c>';}
function tkb_download_template(?array $week,array $classes,string $filename):never{
    if(!class_exists('ZipArchive')){http_response_code(500);exit('Máy chủ chưa bật ZipArchive để tạo file Excel.');}
    $tmp=tempnam(sys_get_temp_dir(),'tkb_xlsx_');$zip=new ZipArchive();if($tmp===false||$zip->open($tmp,ZipArchive::CREATE|ZipArchive::OVERWRITE)!==true){http_response_code(500);exit('Không tạo được file Excel mẫu.');}
    $rows=tkb_template_rows($week,$classes);$sheet='<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetData>';
    foreach($rows as$ri=>$values){$rowNo=$ri+1;$sheet.='<row r="'.$rowNo.'">';foreach($values as$ci=>$value)$sheet.=tkb_xlsx_cell($ci+1,$rowNo,(string)$value);$sheet.='</row>';}$sheet.='</sheetData></worksheet>';
    ...
    $zip->addFromString('xl/workbook.xml','<?xml version="1.0" encoding="UTF-8"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="TKB" sheetId="1" r:id="rId1"/></sheets></workbook>');
    $zip->addFromString('xl/_rels/workbook.xml.rels','<?xml version="1.0" encoding="UTF-8"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/></Relationships>');
    $zip->addFromString('xl/worksheets/sheet1.xml',$sheet);$zip->close();
    header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');header('Content-Disposition: attachment; filename="'.preg_replace('/[^A-Za-z0-9._-]/','_',$filename).'"');header('Content-Length: '.filesize($tmp));readfile($tmp);@unlink($tmp);exit;
}
