...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'settings' && $admin && in_array($cat, ['tn','ts'], true)) {
        ...
        } elseif (in_array($action, ['member','bulk_add','bulk_remove'], true) && in_array($cat, ['tn','ts','muinhon','chuadat'], true) && in_array($subject,$available,true)) {
            ...
            $ids = $action === 'member' ? [(string)($_POST['student_id'] ?? '')] : array_values(array_unique(array_map('strval', (array)($_POST['student_ids'] ?? []))));
...
$viewStudents=array_filter($students,static function($s) use($class,$subject,$canSelect) { return (!$class || $s['class']===$class) && $subject!=='' && $canSelect($s,$subject); });
if (!$admin && $subject !== '') $allClasses = array_values(array_filter($allClasses, static function($cl) use($norm,$allowed,$subject) { return isset($allowed[$norm($cl)][$norm($subject)]); }));
...
<div class="card"><div class="card-body"><h5><?=e($tabs[$tab])?> · <?=e($subject)?> <span class="badge text-bg-primary"><?=count($members[$tab][$subject]??[])?></span></h5><p class="text-muted small"><?=in_array($tab,['tn','ts'],true)?'Giáo viên đánh dấu học sinh đăng ký môn thi ở lớp, môn mình phụ trách.':'Giáo viên đánh dấu học sinh ở lớp và môn mình dạy.'?></p><?php if($subject!=='' && ($admin || $subject!=='')):?>
