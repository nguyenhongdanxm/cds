...
require_login();
if(!lb_is_admin()){http_response_code(403);exit('Chỉ quản trị được nhập PPCT.');}
...
$result=lb_ppct_import_v2($_FILES['curriculum_file']??[]);flash($result['message'],$result['ok']?'success':'danger');header('Location: '.BASE_URL.'sodaubai.php?tab=curriculum&week='.urlencode((string)($_POST['week']??'')));exit;
