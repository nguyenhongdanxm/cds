...
// Chỉ Quản trị hoặc tài khoản được phân quyền quản lý/PCCM mới được tra cứu tải giảng toàn trường.
$canDailyLoad=$isAdmin||cds_can_feature('cm.pccm','edit')||cds_can_feature('cm.nhaplieu','edit');
if(!$canDailyLoad){http_response_code(403);exit('Bạn chưa được phân quyền xem tải giảng giáo viên theo ngày.');}
...
foreach($slots as $s){
 if((int)($s['day']??0)!==$day)continue;
 $teacher=trim((string)($s['teacher']??''));if($teacher===''||!array_key_exists($teacher,$counts))continue;
 $sub=tkb_substitution_for_slot($week,$s);$actual=$teacher;
 if(is_array($sub)&&($sub['status']??'')==='approved'&&($sub['date']??'')===$date){$replacement=trim((string)($sub['substitute_teacher']??''));if($replacement!=='')$actual=$replacement;}
...
<div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3"><div><h3 class="fw-bold text-primary mb-1"><i class="bi bi-person-lines-fill"></i> Tải giảng giáo viên theo ngày</h3><div class="text-muted">Tổng hợp số tiết lên lớp của từng giáo viên trong cả ngày, không tách Sáng/Chiều.</div></div><a class="btn btn-outline-primary" href="<?=BASE_URL?>thoikhoabieu.php?tab=lookup"><i class="bi bi-arrow-left"></i> Thời khóa biểu</a></div>
<form class="card p-3 mb-3" method="get"><div class="row g-2 align-items-end"><div class="col-md-4"><label class="form-label fw-semibold">Ngày xem</label><input class="form-control" type="date" name="date" value="<?=e($date)?>"></div><div class="col-md-4"><label class="form-label fw-semibold">Tổng số tiết trong ngày</label><select class="form-select" name="lessons"><option value="all">Tất cả giáo viên</option><?php for($i=0;$i<=8;$i++):?><option value="<?=$i?>" <?=$countFilter===(string)$i?'selected':''?>><?=$i===0?'Không có tiết':$i.' tiết'?></option><?php endfor;?></select></div><div class="col-md-4"><button class="btn btn-primary w-100"><i class="bi bi-funnel"></i> Lọc danh sách</button></div></div></form>
<div class="row g-2 mb-3"><?php for($i=0;$i<=4;$i++):?><div class="col-6 col-md"><a class="text-decoration-none" href="?date=<?=urlencode($date)?>&lessons=<?=$i?>"><div class="load-chip <?=$i===0?'load-zero':''?>"><strong><?=intval($summary[$i]??0)?></strong><span><?=$i===0?'Không có tiết':$i.' tiết/ngày'?></span></div></a></div><?php endfor;?></div>
<div class="card overflow-hidden"><div class="card-header d-flex justify-content-between"><span>Ngày <?=date('d/m/Y',strtotime($date))?></span><span><?=count($rows)?> giáo viên</span></div><div class="table-responsive"><table class="table table-hover load-table mb-0"><thead><tr><th style="width:55px">STT</th><th>Giáo viên</th><th class="text-center">Tổng số tiết</th><th>Các tiết lên lớp trong ngày</th></tr></thead><tbody><?php if(!$rows):?><tr><td colspan="4" class="text-center text-muted py-4">Không có giáo viên phù hợp bộ lọc.</td></tr><?php endif;?><?php foreach($rows as$i=>$r):?><tr><td><?=($i+1)?></td><td class="fw-semibold"><?=e($r['teacher'])?></td><td class="text-center"><span class="badge <?=$r['count']===0?'text-bg-warning':'text-bg-primary'?> fs-6"><?=$r['count']?></span></td><td><?php if(!$r['details']):?><span class="text-muted">Không có tiết lên lớp</span><?php else:?><?php foreach($r['details'] as$s):?><span class="lesson-pill">Tiết <?=intval($s['period']??0)?> · <?=e((string)($s['class']??$s['class_raw']??''))?> · <?=e((string)($s['subject']??''))?></span><?php endforeach;?><?php endif;?></td></tr><?php endforeach;?></tbody></table></div></div>
</div>
