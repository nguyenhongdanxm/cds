...
?>
<style>
.lesson-sheet{max-width:920px;margin:auto;background:#fff;padding:22px 30px;border-radius:12px;box-shadow:0 3px 16px rgba(15,23,42,.09);font-size:13pt}.sheet-school{font-family:"Times New Roman",serif;font-size:13pt;line-height:1.15;margin-bottom:13pt}.sheet-title{text-align:center;font:bold 16pt "Times New Roman",serif;margin:0 0 13pt}.sheet-meta{display:grid;grid-template-columns:1fr;gap:1px;margin-bottom:7px;font-family:"Times New Roman",serif;font-size:13pt;line-height:1.1}.sheet-meta>div{border-bottom:0;padding:1px 0}.meta-triplet{display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr));gap:20px}.score-table{width:100%;border-collapse:collapse;font-family:"Times New Roman",serif;font-size:13pt;line-height:1.08}.score-table th,.score-table td{border:.5pt solid #444;padding:2px 4px;vertical-align:middle}.score-table th{text-align:center}.score-table .group{width:104px;font-weight:bold;text-align:center}.score-table .criterion{text-align:left;font-size:12.5pt}.score-table .max,.score-table .score{width:74px;text-align:center}.score-table input{width:58px;text-align:center;padding:2px;border:1px solid #aab4c3;border-radius:4px;font-size:13pt}.sheet-signatures{display:grid;grid-template-columns:1fr 1fr;text-align:center;margin-top:2mm;font-family:"Times New Roman",serif;font-size:13pt;line-height:1.1}.signature-date{min-height:16px;font-style:italic}.signature-space{height:41px}.signature-name{font-weight:bold}.sheet-actions{display:flex;flex-wrap:wrap;gap:8px;margin:0 auto 12px;max-width:920px}.sheet-observer-tabs{display:flex;flex-wrap:wrap;gap:6px;margin-bottom:7px}.sheet-result{min-height:7mm;display:flex;align-items:center;justify-content:center;text-align:center;margin:0;font:700 13pt "Times New Roman",serif;flex:none}.sheet-save{text-align:center;margin-top:7px}.pdf-mode{width:170mm!important;max-width:170mm!important;height:257mm!important;padding:0!important;border-radius:0!important;box-shadow:none!important;font-size:13pt!important;display:flex!important;flex-direction:column!important}.pdf-mode .no-print{display:none!important}.pdf-mode form{display:flex;flex:1;min-height:0;flex-direction:column}.pdf-mode .sheet-table-wrap{display:flex;flex:1;min-height:0}.pdf-mode .score-table{height:100%}.pdf-mode .sheet-school,.pdf-mode .sheet-meta,.pdf-mode .score-table,.pdf-mode .sheet-signatures,.pdf-mode .sheet-result{font-size:13pt!important}.pdf-mode .score-table .criterion{font-size:12.5pt!important}.pdf-mode .score-table input{border:0;padding:0;font:inherit}
...
document.getElementById('savePdfDrive')?.addEventListener('click',async function(){
  ...
  button.disabled=true;button.innerHTML='Đang lưu…';try{const pdf=await createLessonPdf(),data=new FormData();data.append('drive_api','save');data.append('csrf','<?=e(cds_drive_csrf_token())?>');data.append('source_action','<?=e(cds_drive_page_action())?>');data.append('type','observations');data.append('name','phieu-du-gio-<?=preg_replace('/[^a-zA-Z0-9_-]+/','-',trim((string)($record['id']??'du-gio')))?>.pdf');data.append('file',pdf.output('blob'),'phieu-du-gio.pdf');const response=await fetch('../admin.php',{method:'POST',body:data});const result=await response.json();alert(result.ok?'Đã lưu phiếu dự giờ lên Google Drive.':(result.message||'Không lưu được lên Drive.'));}catch(error){console.error(error);alert('Không lưu được phiếu lên Drive.');}finally{button.disabled=false;button.innerHTML=oldText;}
});
</script>
<?php endif;?>
