...
function td_room_week(string $date): array {
    ...
    $weeks=csdl_year_weeks();
    if ($weeks) {
        $selected=end($weeks);
        foreach ($weeks as $week) { $selected=$week; if (($week['start']??'') >= $date) break; }
        return $selected;
    }
    $time=strtotime($date)?:time(); $start=date('Y-m-d',strtotime('monday this week',$time));
    return ['start'=>$start,'end'=>date('Y-m-d',strtotime($start.' +6 days')),'label'=>'Tuần chưa cấu hình'];
}
...
$roomWeeks=csdl_year_weeks(); $today=date('Y-m-d'); $weekRequest=(string)($_GET['week']??$today); $inputWeek=td_room_week($weekRequest); $inputDateDefault=($today>=$inputWeek['start']&&$today<=$inputWeek['end'])?$today:$inputWeek['start']; $inputMap=[];
...
<?php if($tab==='input'):?>
<form class="cardx toolbar no-print" method="get"><input type="hidden" name="tab" value="input"><div><label class="form-label">Tuần chấm (đồng bộ CSDL)</label><select class="form-select" name="week"><?php foreach($roomWeeks as $weekOption):?><option value="<?=e($weekOption['start'])?>" <?=($weekOption['start']??'')===($inputWeek['start']??'')?'selected':''?>><?=e(csdl_week_display($weekOption))?></option><?php endforeach;?></select></div><button class="btn btn-warning"><i class="bi bi-calendar-week"></i> Xem tuần</button><div class="ms-auto"><strong><?=e(csdl_week_display($inputWeek))?></strong><div class="small text-muted">Theo tuần đã cài đặt trong CSDL · Điểm chuẩn <?=number_format((float)$data['settings']['weekly_max'],1,',','.')?></div></div></form>
