...
$setId = trim((string)($_GET['set'] ?? ''));
$quizSet = null;
if ($setId !== '') {
    try { qp_schema(); $quizSet = qp_set($setId, true); } catch (Throwable $e) { $quizSet = null; }
}
$bankJson = json_encode($quizSet ? qp_questions($quizSet) : [], JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT);
?>
...
<main><div class="panel no-print"><form method="get"><?php if ($quizSet): ?><input type="hidden" name="set" value="<?=e($setId)?>"><?php endif; ?><label for="class">Lớp chơi</label><select id="class" name="class" onchange="this.form.submit()"><option value="">Chọn lớp</option><?php foreach ($classes as $class): ?><option value="<?=e((string)$class['id'])?>" <?=($chosen && $class['id']===$chosen['id'])?'selected':''?>><?=e((string)$class['name'])?></option><?php endforeach; ?></select></form></div>
<?php if ($chosen): ?>
...
<script>
const students=<?=$studentJson?:'[]'?>, classId=<?=json_encode($classId,JSON_HEX_TAG|JSON_HEX_AMP|JSON_HEX_APOS|JSON_HEX_QUOT)?>,bank=<?=$bankJson?:'[]'?>,setId=<?=json_encode($quizSet?$setId:'',JSON_HEX_TAG|JSON_HEX_AMP|JSON_HEX_APOS|JSON_HEX_QUOT)?>;
const byId=new Map(students.map(s=>[s.id,s]));
const $=id=>document.getElementById(id), escapeHtml=s=>String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const storageKey='cds-qr-trial-v2:'+classId+':'+setId;
...
if(setId){state.questions=bank.map((q,i)=>({...q,answers:state.questions[i]?.answers||{}}));if(state.index<0&&bank.length)state.index=0}
