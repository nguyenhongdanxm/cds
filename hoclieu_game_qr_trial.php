...
let cameraStream=null,scanning=false,busy=false,lastSeen=new Map(),paperOpen=true;
...
async function syncQuestion(){if(!paperCode)return;try{let response=await fetch(screenApi+'?code='+encodeURIComponent(paperCode)+'&api=state',{cache:'no-store'}),data=await response.json();if(!data.ok)throw Error('Không đọc được lượt chơi');paperOpen=data.status==='open';if(!paperOpen){stopCamera();$('scanStatus').textContent='Lượt chơi đã đóng.';return}if(state.index!==data.index){state.index=data.index;lastSeen.clear();save();render();$('scanStatus').textContent='Đang quét câu '+(data.index+1)}}catch(e){$('scanStatus').textContent='Mất kết nối màn hình chiếu: '+e.message}}
...
$('startScan').onclick=startCamera;$('stopScan').onclick=stopCamera;window.addEventListener('pagehide',stopCamera);render();if(paperCode){syncQuestion();setInterval(syncQuestion,2500)}
