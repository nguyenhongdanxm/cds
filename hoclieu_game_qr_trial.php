...
$quizSet = null;
if ($setId !== '') {
    try { qp_schema(); $quizSet = qp_set($setId, true); } catch (Throwable $e) { $quizSet = null; }
}
...
<main><div class="panel no-print"><?php if ($paperSession): ?><strong>Lượt thẻ giấy <?=e($code)?> · Lớp <?=e((string)($chosen['name']??''))?></strong><?php else: ?><form method="get"><?php if ($quizSet): ?><input type="hidden" name="set" value="<?=e($setId)?>"><?php endif; ?><label for="class">Lớp chơi</label><select id="class" name="class" onchange="this.form.submit()"><option value="">Chọn lớp</option><?php foreach ($classes as $class): ?><option value="<?=e((string)$class['id'])?>" <?=($chosen && $class['id']===$chosen['id'])?'selected':''?>><?=e((string)$class['name'])?></option><?php endforeach; ?></select></form><?php endif; ?></div>
<?php if ($chosen): ?>
...
<div class="grid no-print"><section class="panel"><h2>1. Câu hỏi<?php if ($quizSet): ?> · <?=e((string)$quizSet['title'])?><?php endif; ?></h2><label for="question">Nội dung</label><textarea id="question" placeholder="Nhập câu hỏi để hiển thị khi chơi" <?=$quizSet?'readonly':''?>></textarea><div id="choices"></div><label>Đáp án đúng</label><div class="answer-buttons" id="keys"></div><button id="newQuestion" <?=$quizSet?'class="hidden"':''?>>Mở câu hỏi mới</button><?php if ($quizSet): ?><button id="nextQuestion">Câu tiếp theo</button><?php endif; ?><button id="clearQuestion" class="secondary">Xóa lượt quét của câu này</button><p id="current" class="status"></p></section>
...
if(setId)$('nextQuestion').onclick=()=>{if(state.index<state.questions.length-1){stopCamera();state.index++;save();render()}else $('scanStatus').textContent='Đã hết bộ câu hỏi.'};
$('clearQuestion').onclick=()=>{let q=current();if(q&&confirm('Xóa tất cả câu trả lời của câu hiện tại?')){let ids=Object.keys(q.answers);q.answers={};save();render();ids.forEach(id=>persistAnswer(id,'',state.index))}};
$('resultRows').onchange=e=>{if(!e.target.matches('.manual'))return;let q=current();if(!q)return;let id=e.target.dataset.id;if(!byId.has(id))return;if(e.target.value)q.answers[id]=e.target.value;else delete q.answers[id];save();render();persistAnswer(id,e.target.value,state.index)};
