...
<div class="section-head"><div class="quick-actions number-book-tabs"><a class="number-book-tab all <?=$numberBook==='all'?'active':''?>" href="?<?=e(http_build_query(['tab'=>'numbers','number_book'=>'all','number_q'=>(string)($_GET['number_q']??''),'number_sort'=>$numberSort,'number_dir'=>$numberDir]))?>">Tất cả <b><?=count($numbers)?></b></a><a class="number-book-tab decision <?=$numberBook==='decision'?'active':''?>" href="?<?=e(http_build_query(['tab'=>'numbers','number_book'=>'decision','number_q'=>(string)($_GET['number_q']??''),'number_sort'=>$numberSort,'number_dir'=>$numberDir]))?>">Quyết định <b><?=count(array_filter($numbers,fn($r)=>($r['book']??'')==='decision'))?></b></a><a class="number-book-tab other <?=$numberBook==='other'?'active':''?>" href="?<?=e(http_build_query(['tab'=>'numbers','number_book'=>'other','number_q'=>(string)($_GET['number_q']??''),'number_sort'=>$numberSort,'number_dir'=>$numberDir]))?>">Văn bản khác <b><?=count(array_filter($numbers,fn($r)=>($r['book']??'')==='other'))?></b></a></div><?php if($canNumber):?><button class="btn btn-primary" onclick="document.getElementById('numberDialog').showModal()"><i class="bi bi-plus-lg"></i> Lấy số mới</button><?php endif;?></div>
