...
$canEngagementManage = can_perm_level('vb.tuongtac', 'edit') || $canManage;
$tab = (string)($_GET['tab'] ?? 'overview');
if (!in_array($tab, ['overview','documents','numbers','archives','polls','surveys','feedback'], true)) $tab = 'overview';
...
$nav = [
    ...
    'documents'=>['Văn bản lưu trữ','bi-file-earmark-text-fill',true],
    'archives'=>['Hồ sơ lưu trữ','bi-archive-fill',true],
    'polls'=>['Bình chọn','bi-check2-circle',true],
    'surveys'=>['Khảo sát','bi-ui-checks-grid',true],
...
@media(max-width:900px){.engage-grid{grid-template-columns:1fr}.reply-form{grid-template-columns:1fr}.mobile-nav{display:flex!important;overflow-x:auto;justify-content:flex-start}.mobile-nav a{flex:0 0 78px}.mobile-nav a.locked{opacity:.38;pointer-events:none}}
</style></head><body>
