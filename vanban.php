...
.table{font-size:.86rem;line-height:1.3}.table td{padding:.58rem .65rem}.table .title{line-height:1.25}.table .sub{line-height:1.25;margin-top:.12rem}.table .row-actions{flex-wrap:nowrap}.table .btn{padding:.44rem .58rem;font-size:.78rem}
...
if(numberDialog){
  ...
  function suggestedSymbol(){
    ...
    let max=0,suffix='';
    rows.forEach(row=>{if(row.book!==book||parseInt(row.year||0,10)!==year)return;const number=parseInt(row.number||String(row.symbol||'').match(/^\\s*(\\d+)/)?.[1]||0,10);if(number>=max){max=number;const match=String(row.symbol||'').match(/^\\s*\\d+(.*)$/);if(match)suffix=match[1]||''}});
    if(!suffix){let latest=0;rows.forEach(row=>{if(row.book!==book)return;const number=parseInt(row.number||0,10);if(number>=latest){const match=String(row.symbol||'').match(/^\\s*\\d+(.*)$/);if(match&&match[1]){latest=number;suffix=match[1]}}})}
    if(!suffix)suffix=book==='decision'?'/QĐ-PTDTNT':'/PTDTNT';
