...
$availableGrades=[];foreach($boarders as $student){$grade=noitru_assignment_numeric_grade($student);if($grade>0)$availableGrades[$grade]=true;}$availableGrades=array_keys($availableGrades);sort($availableGrades,SORT_NUMERIC);

if($mode==='rooms'&&($_GET['download_template']??'')==='rooms')nt_room_excel_template(array_keys($knownGroups));

if(($_SERVER['REQUEST_METHOD']??'')==='POST'){
    $action=(string)($_POST['action']??'');
    if($action==='import_rooms_excel'){
        if($mode!=='rooms')flash('Chức năng Excel này chỉ dùng để nhập chia phòng.','danger');
        elseif(!isset($_FILES['room_excel'])||($_FILES['room_excel']['error']??UPLOAD_ERR_NO_FILE)!==UPLOAD_ERR_OK)flash('Hãy chọn file Excel XLSX hợp lệ.','danger');
        else{
            ...
            if($extension!=='xlsx')flash('Chỉ chấp nhận file .xlsx tải từ mẫu của hệ thống.','danger');
            elseif($size<=0||$size>10*1024*1024)flash('File Excel phải có dung lượng từ 1 byte đến 10 MB.','danger');
            else{
                $parsed=nt_room_excel_parse((string)$upload['tmp_name']);
                if(empty($parsed['ok']))flash('Không thể nhập: '.implode(' | ',array_slice($parsed['errors']??['File không hợp lệ.'],0,8)).(count($parsed['errors']??[])>8?' | ...':''),'danger');
                else{
                    $result=nt_room_excel_match_and_apply($parsed,$boarders,$data,($_POST['import_scope']??'merge')==='replace_all');
                    if(empty($result['ok']))flash('Chưa lưu vì có lỗi: '.implode(' | ',array_slice($result['errors']??[],0,8)).(count($result['errors']??[])>8?' | ...':''),'danger');
                    else{$data['history'][]=['mode'=>'rooms','action'=>'import_excel','by'=>$by,'at'=>date('c'),'count'=>$result['count'],'rooms'=>$result['rooms'],'replace_all'=>(($_POST['import_scope']??'merge')==='replace_all')];noitru_assignments_save($data,$by);flash('Đã nhập chia phòng từ Excel: '.$result['count'].' học sinh vào '.$result['rooms'].' phòng.','success');}
                }
            }
        }
    }elseif($action==='create_groups'){
        ...
        if(!$names)flash('Hãy tạo danh sách '.$label.' trước.','danger');
            ...
            if($mode==='rooms'){
                $selectedGrades=array_values(array_unique(array_filter(array_map('intval',$_POST['selected_grades']??[]),static fn($grade)=>$grade>0)));
                if(!$selectedGrades){flash('Hãy chọn ít nhất một khối cho đợt chia phòng.','danger');header('Location: '.BASE_URL.'noitru_assign.php?mode=rooms');exit;}
                $stageMode=(($_POST['stage_mode']??'merge')==='replace')?'replace':'merge';
                $candidates=[];$seedSlots=array_fill_keys($names,[]);
                foreach($boarders as $student){
                    $studentGrade=noitru_assignment_numeric_grade($student);$currentRoom=trim((string)($student[$field]??''));$inStage=in_array($studentGrade,$selectedGrades,true);
                    $shouldAssign=$inStage&&($stageMode==='replace'||$currentRoom==='');
                    if($shouldAssign)$candidates[]=$student;
                    elseif($currentRoom!==''&&isset($seedSlots[$currentRoom]))$seedSlots[$currentRoom][]=$student;
                }
                $options=['enforce_gender'=>isset($_POST['enforce_gender']),'max_grade_gap'=>max(0,min(3,(int)($_POST['max_grade_gap']??1))),'seed_slots'=>$seedSlots,'selected_grades'=>$selectedGrades,'stage_mode'=>$stageMode];
                $data['settings']['room_enforce_gender']=$options['enforce_gender'];$data['settings']['room_max_grade_gap']=$options['max_grade_gap'];
                $data['settings']['room_selected_grades']=$selectedGrades;$data['settings']['room_stage_mode']=$stageMode;
                $map=noitru_assignment_auto_rooms($candidates,$names,$caps,$data['room_genders']??[],$options);
            }else{
            ...
            // Chia phòng theo đợt chỉ thay đổi học sinh thuộc đợt hiện tại; chia mâm vẫn chia lại toàn bộ.
            foreach(($mode==='rooms'?$candidates:$boarders) as $student)$data[$mode][(string)$student['id']]='';
            foreach($map as $studentId=>$target)$data[$mode][$studentId]=$target;
            $candidateCount=$mode==='rooms'?count($candidates):count($boarders);$notAssigned=$candidateCount-count($map);$historyOptions=$options;unset($historyOptions['seed_slots']);$data['history'][]=['mode'=>$mode,'action'=>$mode==='rooms'?'auto_stage':'auto','by'=>$by,'at'=>date('c'),'count'=>count($map),'unassigned'=>$notAssigned,'options'=>$historyOptions];
            $stageText=$mode==='rooms'?' khối '.implode(', ',$selectedGrades).($stageMode==='merge'?' (ghép tiếp)':' (chia lại)'):'';
            noitru_assignments_save($data,$by);flash('Đã tự động chia '.count($map).' học sinh'.$stageText.($notAssigned>0?'; còn '.$notAssigned.' học sinh trong đợt chưa có chỗ phù hợp':'').'.',$notAssigned>0?'warning':'success');
        }
...
$roomGenders=is_array($data['room_genders']??null)?$data['room_genders']:[];$roomNotes=is_array($data['room_notes']??null)?$data['room_notes']:[];$settings=is_array($data['settings']??null)?$data['settings']:[];
if(($_GET['export']??'')==='a4'){require __DIR__.'/includes/noitru_assignment_export.php';exit;}
...
<div class="nt-page-head"><div><h4 class="mb-0 fw-bold"><i class="bi <?=$mode==='rooms'?'bi-door-closed':'bi-egg-fried'?>"></i> <?=e($page_title)?></h4><div class="text-muted small mt-1">Quản lý danh sách, sức chứa, chia tự động, gán nhanh và xóa chọn.</div></div><div class="d-flex gap-2 flex-wrap"><?php if($mode==='rooms'):?><a class="btn btn-sm btn-outline-success" href="?mode=rooms&download_template=rooms"><i class="bi bi-file-earmark-excel"></i> Tải mẫu Excel</a><?php endif;?><a class="btn btn-sm btn-success" href="?mode=<?=e($mode)?>&export=a4" target="_blank"><i class="bi bi-file-earmark-pdf"></i> Xuất A4 theo <?=e($label)?></a><a class="btn btn-sm <?=$mode==='rooms'?'btn-primary':'btn-outline-primary'?>" href="?mode=rooms">Chia phòng</a><a class="btn btn-sm <?=$mode==='meals'?'btn-warning':'btn-outline-warning'?>" href="?mode=meals">Chia mâm</a><a class="btn btn-sm btn-outline-secondary" href="<?=BASE_URL?>noitru_list.php?view=<?=$mode?>">Trở lại</a></div></div>
<?php show_flash(); ?>
<?php if($mode==='rooms'):?><div class="assign-card p-3 mb-3"><div class="row g-3 align-items-end"><div class="col-lg-5"><h6 class="fw-bold mb-1"><i class="bi bi-file-earmark-excel text-success"></i> Nhập chia phòng bằng Excel</h6><div class="small text-muted">Một bảng gồm 7 cột: STT, Họ và tên, Lớp, Ngày sinh, Giới tính, Phòng KTX, Ghi chú. Hệ thống vẫn đọc được mẫu cũ theo từng sheet.</div></div><form method="post" enctype="multipart/form-data" class="col-lg-7 row g-2 align-items-end"><input type="hidden" name="mode" value="rooms"><input type="hidden" name="action" value="import_rooms_excel"><div class="col-md-5"><label class="form-label small">File Excel .xlsx</label><input class="form-control form-control-sm" type="file" name="room_excel" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" required></div><div class="col-md-4"><label class="form-label small">Cách nhập</label><select class="form-select form-select-sm" name="import_scope"><option value="merge">Ghép/cập nhật, giữ kết quả khác</option><option value="replace_all">Xóa kết quả cũ rồi nhập lại</option></select></div><div class="col-md-3"><button class="btn btn-sm btn-success w-100" onclick="return confirm('Nhập kết quả chia phòng từ file Excel?')"><i class="bi bi-upload"></i> Kiểm tra và nhập</button></div></form></div></div><?php endif;?>
...
<div class="assign-groups"><?php foreach($grouped as $groupName=>$students):$sum=noitru_assignment_summary($students);$roomGender=(string)($roomGenders[$groupName]??'Linh hoạt');?><section class="assign-card overflow-hidden" data-assigned="1" data-group="<?=e($groupName)?>"><div class="assign-group-head"><div class="d-flex justify-content-between gap-2"><div><strong><?=e($groupName)?></strong><?php if($mode==='rooms'):?> <span class="badge gender-<?=e(mb_strtolower(str_replace(' ','-',$roomGender),'UTF-8'))?>"><?=e($roomGender)?></span><?php endif;?></div><div class="d-flex align-items-center gap-1"><button type="button" class="btn btn-sm btn-link py-0 select-group" title="Chọn toàn bộ học sinh trong <?=e($groupName)?>">Chọn cả</button><span class="badge bg-primary"><?=$sum['total']?> / <?=(int)($capacities[$groupName]??0)?> HS</span></div></div><div class="assign-summary">Nam <?=$sum['male']?> · Nữ <?=$sum['female']?> · Khối <?=e(implode(', ',array_keys($sum['grades'])))?:'—'?> · Lớp <?=e(implode(', ',array_keys($sum['classes'])))?:'—'?></div></div><div class="assign-scroll"><?php foreach($students as $student):$studentNote=trim((string)($roomNotes[(string)$student['id']]??''));?><div class="assign-student" data-class="<?=e($student['class_name'])?>" data-gender="<?=e(noitru_assignment_gender($student))?>" data-status="assigned" data-name="<?=e(mb_strtolower($student['name'],'UTF-8'))?>"><input form="bulkAssignForm" class="form-check-input mt-1 assign-check" type="checkbox" name="student_ids[]" value="<?=e($student['id'])?>" id="s_<?=e($student['id'])?>"><label for="s_<?=e($student['id'])?>"><div class="assign-name"><?=e($student['name'])?></div><div class="assign-meta"><?=e($student['class_name'])?> · <?=e(noitru_assignment_gender($student))?> · Khối <?=e(noitru_assignment_grade($student))?><?php if($mode==='rooms'&&$studentNote!==''):?> · Ghi chú: <?=e($studentNote)?><?php endif;?></div></label></div><?php endforeach;?><?php if(!$students):?><div class="text-muted text-center py-3 small">Chưa có học sinh.</div><?php endif;?></div></section><?php endforeach;?>
