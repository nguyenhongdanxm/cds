...
$path=(string)$files[$index]['path'];$name=(string)$files[$index]['name'];$file=vb_read_stored_file($path,$name);if(empty($file['ok']))vp_fail('Không tìm thấy tệp trên máy chủ hoặc Google Drive.',404);$bytes=(string)$file['body'];
$resolvedName=(string)($file['name']??'');$ext=strtolower((string)pathinfo($name,PATHINFO_EXTENSION));if($ext==='')$ext=strtolower((string)pathinfo($resolvedName,PATHINFO_EXTENSION));$mime=strtolower((string)($file['mime']??''));
if($ext==='pdf'||str_contains($mime,'application/pdf')){header('Content-Type: application/pdf');header('Content-Length: '.strlen($bytes));header('Content-Disposition: inline; filename*=UTF-8\'\''.rawurlencode($name?:$resolvedName));header('Cache-Control: private, max-age=300');echo $bytes;exit;}
if(in_array($ext,['png','jpg','jpeg','gif','webp'],true)||str_starts_with($mime,'image/')){$imageMime=str_starts_with($mime,'image/')?$mime:($ext==='png'?'image/png':($ext==='gif'?'image/gif':($ext==='webp'?'image/webp':'image/jpeg')));header('Content-Type: '.$imageMime);header('Content-Length: '.strlen($bytes));header('Content-Disposition: inline; filename*=UTF-8\'\''.rawurlencode($name?:$resolvedName));header('Cache-Control: private, max-age=300');echo $bytes;exit;}
