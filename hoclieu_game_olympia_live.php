...
try{
 ...
 if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  $allowed=['status','question_index','stage','question','answer','points','seconds','timer_ends_at','stage_score','total_score'];$clean=[];foreach($allowed as $key)if(array_key_exists($key,$state))$clean[$key]=$state[$key];$clean['updated_ms']=(int)round(microtime(true)*1000);
