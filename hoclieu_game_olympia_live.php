...
try{
 ...
 if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  $allowed=['status','question_index','stage','question','answer','points','seconds','timer_ends_at','stage_score','total_score','question_score','awarded_students','rankings','remaining_stages','teacher_name','class_name','event_id','sent_at','play_session_id'];$clean=[];foreach($allowed as $key)if(array_key_exists($key,$state))$clean[$key]=$state[$key];$clean['updated_ms']=(int)round(microtime(true)*1000);
  $u=current_user()??[];$name=(string)($u['teacher_name']??$u['name']??$u['username']??'');$json=json_encode($clean,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);$s=$db->prepare('INSERT INTO cds_olympia_live_sessions(week_id,class_id,state_json,controlled_by) VALUES(?,?,?,?) ON DUPLICATE KEY UPDATE state_json=VALUES(state_json),controlled_by=VALUES(controlled_by)');$s->execute([$weekId,$classId,$json,$name]);
  $historyId=substr(preg_replace('/[^a-zA-Z0-9_-]/','',(string)($clean['play_session_id']??'')),0,64);if($historyId!==''){$class=(array)(olympia_class_map()[$classId]??[]);$status=(string)($clean['status']??'welcome');$finished=in_array($status,['finished','reset'],true)?date('Y-m-d H:i:s'):null;$h=$db->prepare('INSERT INTO cds_olympia_play_history(id,week_id,class_id,class_name,status,last_state,controlled_by,finished_at) VALUES(?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE status=VALUES(status),last_state=VALUES(last_state),controlled_by=VALUES(controlled_by),finished_at=COALESCE(VALUES(finished_at),finished_at)');$h->execute([$historyId,$weekId,$classId,(string)($class['name']??''),$status,$json,$name,$finished]);}
  echo json_encode(['ok'=>true,'updated_ms'=>$clean['updated_ms']],JSON_UNESCAPED_UNICODE);exit;
 }
