    ...
    try {
        ...
        if ($action === 'open_session') {
            ...
            for ($attempt = 0; $attempt < 3; $attempt++) {
                ...
                try {
                    $st = qp_db()->prepare("INSERT INTO cds_quiz_sessions(code,set_id,class_id,owner_id,questions_json,mode,status,created_at,expires_at) VALUES(?,?,?,?,?,?,'open',NOW(),DATE_ADD(NOW(),INTERVAL 1 DAY))");
                    $st->execute([$code,$setId,$classId,qp_owner(),json_encode(qp_questions($set),JSON_UNESCAPED_UNICODE),$mode]);
                    qp_go($setId);
