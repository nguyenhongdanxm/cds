...
if($_SERVER['REQUEST_METHOD']==='POST'&&isset($_POST['drive_api'])){
    ...
    else{$mime=function_exists('mime_content_type')?(mime_content_type((string)$upload['tmp_name'])?:($upload['type']??'application/octet-stream')):($upload['type']??'application/octet-stream');$result=cds_drive_save_generated($bytes,$name,$mime,$type);}
...
if(($_GET['view']??'')==='drive'){
    ...
    $drive=cds_drive_settings();$oauth=(array)($drive['oauth']??[]);$connected=!empty($oauth['refresh_token']);$history=array_slice(cds_drive_history(),0,30);
