...
if(isset($_GET['drive_file'])){$driveId=(string)$_GET['drive_file'];$etag='"'.sha1($driveId).'"';header('Cache-Control: private, max-age=300');header('ETag: '.$etag);if(trim((string)($_SERVER['HTTP_IF_NONE_MATCH']??''))===$etag){http_response_code(304);exit;}$file=cds_drive_download($driveId);if(empty($file['ok'])){http_response_code((int)($file['status']??404));exit;}header('Content-Type: '.$file['mime']);header('Content-Length: '.strlen($file['body']));$disposition=!empty($_GET['download'])?'attachment':'inline';header("Content-Disposition: $disposition; filename*=UTF-8''".rawurlencode($file['name']));echo $file['body'];exit;}
...
foreach (csdl_classes_all() as $classRow) {
    $classId = trim((string)($classRow['id'] ?? ''));
    if ($classId !== '') $birthdayClassNames[$classId] = trim((string)($classRow['name'] ?? $classId));
}
$birthdayPersonName = static function (array $person) use ($birthdayClassNames): string {
    $name = trim((string)($person['name'] ?? ''));
    if (($person['type'] ?? '') !== 'student') return $name;
    $classId = trim((string)($person['class_id'] ?? ''));
    $className = trim((string)($person['class_name'] ?? ''));
    if ($className === '' && $classId !== '') $className = $birthdayClassNames[$classId] ?? $classId;
    return $name . ($className !== '' ? ' (' . $className . ')' : '');
};
$birthdayTodayTeachers = array_map($birthdayPersonName, $birthdayByRole['teacher']['today']);
$birthdayTodayStudents = array_map($birthdayPersonName, $birthdayByRole['student']['today']);
$birthdayUpcoming = [];
...
<head>
  ...
  <meta name="apple-mobile-web-app-title" content="CDS Xín Mần">
  ...
  <link href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.min.css" rel="stylesheet">
  <link href="<?= e(BASE_URL) ?>assets/admin-dashboard.css?v=20260910-1" rel="stylesheet">
</head>
<body>
  ...
  <div class="header-actions">
    <details class="notification-picker">
      ...
      <div class="notification-menu">
        ...
        <div class="notification-list">
          ...
          <?php endforeach; ?>
          <?php if (!$pushNotifications): ?><div class="notification-empty"><i class="bi bi-bell-slash"></i>Chưa có thông báo</div><?php endif; ?>
        </div>
      </div>
    </details>
    <details class="module-picker">
      ...
      <div class="module-menu">
        ...
        <?php if ($isAdmin): ?><a href="initial_setup.php" style="--module-color:#176a9b"><i class="bi bi-list-check"></i><span>Khởi tạo dữ liệu</span></a><a href="users.php" style="--module-color:#7c3aed"><i class="bi bi-shield-check"></i><span>Phân quyền</span></a><a href="admin.php?view=drive" style="--module-color:#0f9d58"><i class="bi bi-cloud-check"></i><span>Kho Google Drive</span></a><a href="activity.php" style="--module-color:#475569"><i class="bi bi-activity"></i><span>Nhật ký</span></a><?php endif; ?>
      </div>
    </details>
    <details class="user-picker">
      ...
      <div class="user-menu"><a href="admin.php?view=profile"><i class="bi bi-person-vcard"></i>Thông tin cá nhân</a><?php if ($isAdmin): ?><a href="initial_setup.php"><i class="bi bi-list-check"></i>Khởi tạo dữ liệu</a><a href="users.php"><i class="bi bi-person-gear"></i>Tài khoản và quyền</a><a href="admin.php?view=drive"><i class="bi bi-google"></i>Kho Google Drive</a><?php endif; ?><a href="logout.php" class="logout"><i class="bi bi-box-arrow-right"></i>Đăng xuất</a></div>
    </details>
  </div>
</header>
...
<main class="dashboard">
    ...
    <div class="welcome-copy">
      ...
      <?php if ($hasBirthdays): ?>
        <div class="birthday-line"><span class="birthday-icon">🎂</span><div>
          <?php if ($birthdayTodayTeachers): ?><div class="birthday-role-row teacher">Chúc mừng sinh nhật đồng chí <?= e(implode(', ', $birthdayTodayTeachers)) ?></div><?php endif; ?>
          <?php if ($birthdayTodayStudents): ?><div class="birthday-role-row student">Chúc mừng sinh nhật em <?= e(implode(', ', $birthdayTodayStudents)) ?></div><?php endif; ?>
          <?php if ($birthdayUpcoming): ?><div class="birthday-role-row upcoming">Sinh nhật sắp tới: <?= e(implode(', ', $birthdayUpcoming)) ?></div><?php endif; ?>
        </div></div>
  ...
  <div class="content-grid">
    ...
    <?php if(can_module('noitru','view')): ?><section class="panel operation-panel">
      <div class="panel-head"><div><span class="section-kicker">Toàn trường</span><h2>Lịch trực hôm nay</h2></div><?php if($isAdmin):?><a href="admin_operation_settings.php"><i class="bi bi-gear"></i> Cài đặt</a><?php endif;?></div>
      <div class="operation-grid">
        <?php foreach(['management'=>'bi-person-badge-fill','departments'=>'bi-diagram-3-fill','youth'=>'bi-stars'] as $groupKey=>$groupIcon):$groupPeople=$operationGroups[$groupKey]??[];?><article class="operation-group-box"><span class="op-icon"><i class="bi <?=e($groupIcon)?>"></i></span><div class="op-copy"><span><?=e($operationGroupLabels[$groupKey]??'')?></span><?php if($groupPeople):?><div class="operation-people"><?php foreach($groupPeople as$personIndex=>$person):$personNote=trim((string)($person['note']??''),' ()');?><?=$personIndex?', ':''?><b><?=e($person['name']??'')?></b><?php if($personNote!==''):?> <span class="operation-note">(<?=e($personNote)?>)</span><?php endif;?><?php endforeach;?></div><small>Phân công hiển thị hôm nay</small><?php else:?><strong>Chưa phân công</strong><small>Chưa có lịch hiển thị trong ngày</small><?php endif;?></div></article><?php endforeach;?>
        <article class="duty-box"><span class="op-icon"><i class="bi bi-calendar2-check"></i></span><div class="op-copy"><span>Trực nội trú</span><?php if($duty && $duty['people']): ?><strong><?= e(implode(', ',$duty['people'])) ?></strong><small><?= e($duty['start']) ?> – <?= e($duty['end']) ?> hôm sau · còn <?= $dutyHours ?>h <?= $dutyMinutes ?>p</small><?php else: ?><strong>Chưa phân công</strong><small>Chưa có người trực trong ca hiện tại</small><?php endif; ?><?php if($duty && $duty['managers']): ?><em>Quản lý: <?= e(implode(', ',$duty['managers'])) ?></em><?php endif; ?></div></article>
