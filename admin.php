  ...
  <div class="content-grid">
    ...
    <?php if(can_module('noitru','view')): ?><section class="panel operation-panel">
      ...
      <div class="operation-grid">
        <?php foreach(['management'=>'bi-person-badge-fill','departments'=>'bi-diagram-3-fill','youth'=>'bi-stars'] as $groupKey=>$groupIcon):$groupPeople=$operationGroups[$groupKey]??[];?><article class="operation-group-box"><span class="op-icon"><i class="bi <?=e($groupIcon)?>"></i></span><div class="op-copy"><span><?=e($operationGroupLabels[$groupKey]??'')?></span><?php if($groupPeople):?><strong class="operation-people"><?php foreach($groupPeople as$personIndex=>$person):$personNote=trim((string)($person['note']??''),' ()');?><?=$personIndex?', ':''?><?=e($person['name']??'')?><?php if($personNote!==''):?> <i>(<?=e($personNote)?>)</i><?php endif;?><?php endforeach;?></strong><small>Phân công hiển thị hôm nay</small><?php else:?><strong>Chưa phân công</strong><small>Chưa có lịch hiển thị trong ngày</small><?php endif;?></div></article><?php endforeach;?>
