...
if($file===''){$driveId=csdl_student_photo_drive_id($id);if($driveId!==''){$remote=cds_drive_download($driveId);if(!empty($remote['ok'])){header('Content-Type: '.$remote['mime']);header('Content-Length: '.strlen($remote['body']));header('Cache-Control: private, max-age=86400');echo $remote['body'];exit;}}http_response_code(404);exit;}
