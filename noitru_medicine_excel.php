...
function nt_medicine_xlsx_rows(string $file): array {
    ...
    foreach($xml->sheetData->row as $row){
        $cells=[];
        foreach($row->c as $c){
            $ref=(string)$c['r']; preg_match('/([A-Z]+)(\d+)/',$ref,$m);
            $idx=isset($m[1])?nt_medicine_xlsx_col_to_index($m[1]):count($cells);
            $type=(string)$c['t']; $value='';
            if($type==='inlineStr'){$value=(string)$c->is->t;} else {$value=(string)$c->v; if($type==='s')$value=$shared[(int)$value]??'';}
            $cells[$idx]=trim($value);
        }
        if($cells){ksort($cells);$max=max(array_keys($cells));$line=[];for($i=0;$i<=$max;$i++)$line[$i]=$cells[$i]??'';$rows[(int)$row['r']]=$line;}
    }
...
function nt_medicine_norm_header(string $v): string {
    ...
    return preg_replace('/[^a-z0-9]+/','',strtr($v,$map))??'';
}
function nt_medicine_excel_date($raw): string {
    ...
    foreach(['Y-m-d','d/m/Y','d-m-Y','j/n/Y','j/n/y','d/m/y'] as $fmt){$d=DateTime::createFromFormat($fmt,$v);if($d&&$d->format($fmt)===$v)return $d->format('Y-m-d');}
    return '';
}
function nt_medicine_template_xlsx(): void {
    ...
    foreach($all as $ri=>$r){$cells='';foreach($r as $ci=>$v){$col='';$n=$ci+1;while($n>0){$n--;$col=chr(65+$n%26).$col;$n=intdiv($n,26);}$cells.='<c r="'.$col.($ri+1).'" t="inlineStr"><is><t>'.htmlspecialchars((string)$v,ENT_XML1|ENT_QUOTES,'UTF-8').'</t></is></c>';}$rows[]='<row r="'.($ri+1).'">'.$cells.'</row>';}
    $sheet='<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><cols><col min="1" max="1" width="8" customWidth="1"/><col min="2" max="2" width="30" customWidth="1"/><col min="3" max="3" width="14" customWidth="1"/><col min="4" max="4" width="14" customWidth="1"/><col min="5" max="5" width="18" customWidth="1"/><col min="6" max="6" width="36" customWidth="1"/></cols><sheetData>'.implode('',$rows).'</sheetData></worksheet>';
...
$upload=$_FILES['medicine_excel']??null;
if(!$upload||($upload['error']??UPLOAD_ERR_NO_FILE)!==UPLOAD_ERR_OK){flash('Chưa chọn file Excel kho thuốc.','danger');header('Location: '.BASE_URL.'noitru.php?tab=health&health_view=inventory');exit;}
...
$parsed=nt_medicine_xlsx_rows((string)$upload['tmp_name']);
if(empty($parsed['ok'])){flash($parsed['message']??'Không đọc được file Excel.','danger');header('Location: '.BASE_URL.'noitru.php?tab=health&health_view=inventory');exit;}

/* Vẫn nhận file cũ có cột "Ngưỡng sắp hết", nhưng bỏ qua giá trị cột này. */
...
$headerRow=0;$cols=[];
foreach($parsed['rows'] as $rn=>$row){$found=[];foreach($row as $ci=>$v){$k=nt_medicine_norm_header((string)$v);if(isset($aliases[$k]))$found[$aliases[$k]]=$ci;}if(isset($found['name'],$found['unit'],$found['quantity'])){$headerRow=$rn;$cols=$found;break;}}
if(!$headerRow){flash('Sai cấu trúc file. Bắt buộc có các cột: Tên thuốc, Đơn vị, Số lượng.','danger');header('Location: '.BASE_URL.'noitru.php?tab=health&health_view=inventory');exit;}

$items=[];$errors=[];$seen=[];
foreach($parsed['rows'] as $rn=>$row){
    if($rn<=$headerRow)continue;
    $name=trim((string)($row[$cols['name']]??''));
    $unit=trim((string)($row[$cols['unit']]??''));
    $qtyRaw=trim((string)($row[$cols['quantity']]??''));
    if($name===''&&$unit===''&&$qtyRaw==='')continue;
    $rowErrors=[];
    if($name==='')$rowErrors[]='thiếu tên thuốc';
    ...
    if($expiryRaw!==''&&$expiry==='')$rowErrors[]='hạn sử dụng không hợp lệ';
    if($rowErrors){$errors[]='Dòng '.$rn.': '.implode(', ',$rowErrors);continue;}
...
$existing=noitru_medicines_all();$map=[];
foreach($existing as $m)$map[mb_strtolower(trim((string)($m['name']??'')),'UTF-8').'|'.mb_strtolower(trim((string)($m['unit']??'')),'UTF-8')]=$m;
