...
canvas{width:100%;height:100%;filter:drop-shadow(0 16px 28px #0008)}
...
let items=[], angle=0, spinning=false, speed=0, audioCtx=null, lastTick=-1;
...
function fanfare(){ [523,659,784,1046].forEach((f,i)=>setTimeout(()=>beep(f,.18,'triangle',.08), i*90)); }
...
function tick(){ if(!spinning) return; angle+=speed; speed*=0.988; const idx=winnerIndex(); if(idx!==lastTick){ lastTick=idx; beep(220+idx*18,.04,'square',.04); } if(speed<0.0025){ spinning=false; document.getElementById('spinBtn').disabled=false; showWin(items[idx]||'Chưa có dữ liệu'); } draw(); requestAnimationFrame(tick); }
document.getElementById('spinBtn').onclick=function(){ ensureAudio(); items=currentItems(); if(spinning) return; if(items.length<2){ showWin(mode==='student'?'Hãy chọn lớp có từ 2 học sinh.':'Cần ít nhất 2 ô.'); return; } spinning=true; this.disabled=true; speed=0.42+Math.random()*0.28; lastTick=-1; requestAnimationFrame(tick); };
