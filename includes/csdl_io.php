 ...
 *
 * Excel tiếng Việt mặc định tách cột bằng dấu CHẤM PHẨY (;).
 * File xuất dùng ; + bao mọi ô bằng "..." → mở ra đúng từng cột.
 * Nhập tự nhận ; hoặc , hoặc tab.
 */
require_once __DIR__ . '/csdl_store.php';
require_once __DIR__ . '/csdl_schema.php';
require_once __DIR__ . '/csdl_sync.php';

/** Dấu tách cột khi XUẤT (Excel VN) */
define('CSDL_CSV_SEP', ';');
...
function csdl_io_template($entity) {
    ...
    if ($entity === 'teachers') {
        $sample = [1, 'GV001', 'Nguyễn Văn A', '001234567890', '15/03/1985', 'Nam', 'Kinh', '0901234567', 'a@example.com', 'Hà Giang', 'Xín Mần', 'THCS&THPT', 'Toán', 'Tổ Toán', 'Giáo viên', '', '01/09/2010', '3', 'II', 'THCS', '3', '01/01/2024', '', '', '', 'Có', ''];
...
/**
 * @param array $fieldKeys cột schema
 * @param array|null $onlyIds nếu có → chỉ xuất các id này
 */
function csdl_io_export($entity, array $fieldKeys, $onlyIds = null) {
    $schema = csdl_schema_entity($entity);
    ...
    $idSet = null;
    if (is_array($onlyIds) && count($onlyIds) > 0) {
        $idSet = array_flip(array_map('strval', $onlyIds));
    }
    ...
    if ($entity === 'teachers') {
        foreach (csdl_teachers_all() as $t) {
            if ($idSet !== null && !isset($idSet[(string)($t['id'] ?? '')])) continue;
            $i++;
        ...
        $fname = 'csdl-lop-' . date('Ymd-His') . '.csv';
    } else {
        $classes = csdl_classes_all();
        foreach (csdl_students_all() as $s) {
            if ($idSet !== null && !isset($idSet[(string)($s['id'] ?? '')])) continue;
            $i++;
        ...
        $fname = 'csdl-hoc-sinh-' . date('Ymd-His') . '.csv';
    }
    csdl_io_send_csv($fname, $headers, $rows);
}

function csdl_io_detect_delimiter($headerLine) {
    $counts = [
...
function csdl_io_read_csv_file($tmpPath) {
    ...
    // Nếu cả file nằm 1 dòng (mở sai Excel rồi lưu lại) — vẫn cố tách
    $lines = preg_split('/\r\n|\r|\n/', $raw);
    $lines = array_values(array_filter($lines, fn($l) => trim($l) !== ''));
    if (count($lines) < 1) return [null, 'File trống.'];

    $delimiter = csdl_io_detect_delimiter($lines[0]);
    $headers = str_getcsv($lines[0], $delimiter);
    // Bỏ BOM/khoảng trắng thừa trên tiêu đề
...
function csdl_io_map_headers(array $headers, array $schema) {
    ...
    $extra = [
        ...
        'tổ' => 'to_chuyen_mon', 'tổ chuyên môn' => 'to_chuyen_mon',
        'chức vụ' => 'chuc_vu', 'chức vụ (hành chính)' => 'chuc_vu',
        'kiêm nhiệm' => 'kiem_nhiem_text',
        'nội trú' => 'boarder',
        'sđt' => 'phone', 'điện thoại' => 'phone',
        'quê quán' => 'hometown', 'địa chỉ' => 'address',
        'ngày sinh' => 'dob', 'giới tính' => 'gender', 'dân tộc' => 'ethnicity',
        'cấp học' => 'teaching_level',
    ];
...
function csdl_io_import_teachers($tmpPath) {
    ...
    if (!isset($map['name'])) {
        return ['ok' => false, 'message' => 'Thiếu cột «Họ và tên». Kiểm tra dòng tiêu đề (dùng mẫu tải từ hệ thống).'];
    }

    $by = [];
    foreach (csdl_teachers_all() as $t) {
        $by[csdl_norm_name($t['name'] ?? '')] = $t;
    }
    ...
    foreach ($data['rows'] as $row) {
        ...
        $flags = $old['role_flags'] ?? ['is_probation'=>false,'is_principal'=>false,'is_vice'=>false];
        if (isset($map['is_probation'])) $flags['is_probation'] = csdl_io_bool_in(csdl_io_cell($row, $map, 'is_probation'));
        if (isset($map['is_principal'])) $flags['is_principal'] = csdl_io_bool_in(csdl_io_cell($row, $map, 'is_principal'));
        if (isset($map['is_vice'])) $flags['is_vice'] = csdl_io_bool_in(csdl_io_cell($row, $map, 'is_vice'));
        $p['role_flags'] = $flags;
    ...
    return [
        'ok' => true,
        'message' => "GV: thêm $added · cập nhật $updated · bỏ $skipped (giữ chuyên môn/tổ/kiêm nhiệm PCCM đã có)",
...
function csdl_io_import_classes($tmpPath) {
    ...
    $tBy = [];
    foreach (csdl_teachers_all() as $t) $tBy[csdl_norm_name($t['name'] ?? '')] = $t['id'] ?? '';
    $cBy = [];
...
function csdl_io_import_students($tmpPath) {
    ...
    return [
        'ok' => true,
        'message' => "HS: thêm $added · cập nhật $updated · bỏ $skipped (khớp Mã / CCCD / Tên+Lớp)",
        'added' => $added, 'updated' => $updated, 'skipped' => $skipped,
    ];
}
