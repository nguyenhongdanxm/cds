...
/** Trả về sang/trua/toi nếu đây thực sự là một ô điểm danh bữa ăn; buổi điểm danh thường trả về rỗng. */
...
function noitru_att_meal_for_shift($shiftId, $shiftLabel = '') {
    ...
    if (preg_match('/(?:ăn|an)[ _-]*sáng|(?:an|meal)[_-]*sang/u', $text)) return 'sang';
    if (preg_match('/(?:ăn|an)[ _-]*trưa|(?:an|meal)[_-]*trua/u', $text)) return 'trua';
    if (preg_match('/(?:ăn|an)[ _-]*tối|(?:an|meal)[_-]*toi/u', $text)) return 'toi';
    return '';
}
}
