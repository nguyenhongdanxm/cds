...
function cds_drive_token(array $settings=[]): array {$settings=$settings?:cds_drive_settings();$oauth=(array)($settings['oauth']??[]);if(!empty($oauth['client_id'])&&!empty($oauth['client_secret'])&&!empty($oauth['refresh_token'])){$res=cds_drive_http('https://oauth2.googleapis.com/token','POST',['Content-Type: application/x-www-form-urlencoded'],http_build_query(['client_id'=>$oauth['client_id'],'client_secret'=>$oauth['client_secret'],'refresh_token'=>$oauth['refresh_token'],'grant_type'=>'refresh_token']));$json=json_decode($res['body'],true);return (!$res['ok']||empty($json['access_token']))?['ok'=>false,'message'=>$json['error_description']??'Kết nối Google đã hết hiệu lực.']:['ok'=>true,'token'=>$json['access_token'],'provider'=>'oauth'];}$email=trim((string)($settings['client_email']??''));$key=(string)($settings['private_key']??'');if($email===''||$key==='')return ['ok'=>false,'message'=>'Chưa kết nối tài khoản Google.'];$now=time();$head=cds_drive_b64url(json_encode(['alg'=>'RS256','typ'=>'JWT']));$claim=cds_drive_b64url(json_encode(['iss'=>$email,'scope'=>'https://www.googleapis.com/auth/drive','aud'=>'https://oauth2.googleapis.com/token','iat'=>$now-30,'exp'=>$now+3500]));$input=$head.'.'.$claim;if(!openssl_sign($input,$sig,$key,OPENSSL_ALGO_SHA256))return ['ok'=>false,'message'=>'Khóa Service Account không hợp lệ.'];$res=cds_drive_http('https://oauth2.googleapis.com/token','POST',['Content-Type: application/x-www-form-urlencoded'],http_build_query(['grant_type'=>'urn:ietf:params:oauth:grant-type:jwt-bearer','assertion'=>$input.'.'.cds_drive_b64url($sig)]));$json=json_decode($res['body'],true);return (!$res['ok']||empty($json['access_token']))?['ok'=>false,'message'=>$json['error_description']??'Không kết nối được Service Account.']:['ok'=>true,'token'=>$json['access_token'],'provider'=>'service'];}
...
function cds_drive_save_generated(string $bytes,string $filename,string $mime,string $type): array {return cds_drive_upload_bytes($bytes,$filename,$mime,$type,['action'=>'generated']);}
