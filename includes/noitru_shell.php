...
<?php $ntAttRoomMap=[]; if(isset($boarders)&&is_array($boarders)){foreach($boarders as $ntAttStudent){$ntAttSid=(string)($ntAttStudent['id']??'');if($ntAttSid!=='')$ntAttRoomMap[$ntAttSid]=trim((string)($ntAttStudent['room_ktx']??''));}} require __DIR__.'/noitru_att_meal_prefill_inline.php'; ?>
...
<script src="<?= e(BASE_URL . 'assets/noitru_meal_attendance_only.js?v=20260903-6') ?>" defer></script>
<?php endif; ?>
<?php if ($ntPage === 'noitru.php' && $ntTab === 'duty_report'): ?><script src="<?= e(BASE_URL . 'assets/noitru_duty_report_enhancements.js?v=20260903-6') ?>" defer></script><?php endif; ?>
