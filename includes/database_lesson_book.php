...
function cds_lb_import_snapshot($actor=[]){
    $rows=cds_lb_source_rows();$seen=[];$claims=[];foreach($rows as$i=>$row){$row=cds_lb_enrich($row);$id=trim((string)($row['slot_id']??''));if($id===''||($row['date']??'')===''||$row['class_key']===''||$row['subject_key']==='')throw new RuntimeException('Bản ghi '.($i+1).' thiếu khóa chuẩn.');if(isset($seen[$id]))throw new RuntimeException('Trùng mã tiết '.$id.'.');$seen[$id]=true;if(cds_lb_actual_status($row['status']??'')&&(int)($row['ppct_period']??0)>0){$claim=$row['school_year_key'].'|'.$row['class_key'].'|'.$row['subject_key'].'|'.(int)$row['ppct_period'];if(isset($claims[$claim]))throw new RuntimeException('Phát hiện hai bản ghi cùng năm học–lớp–môn–tiết PPCT. Hãy xử lý bản trùng trước khi chuyển MySQL.');$claims[$claim]=true;}}
