...
function cds_dashboard_widgets_catalog(): array {
    return [
        ...
        'csdl' => ['title'=>'Dữ liệu trong phạm vi','icon'=>'bi-database','color'=>'#0f766e','module'=>'csdl','size'=>'normal'],
...
function cds_dashboard_settings(): array {
    ...
    $missing=array_values(array_diff($defaults['order'],$saved['order']??[]));
    $saved['enabled']=array_values(array_unique(array_merge($saved['enabled']??[],$missing)));
    $saved['order']=array_values(array_unique(array_merge($saved['order']??[],$missing)));
