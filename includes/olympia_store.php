...
function olympia_copy_week_content(string $sourceId,string $targetId,bool $copyClasses,bool $copyQuestions): array {
    ...
    if($copyQuestions){$rows=$db->prepare('SELECT stage_name,question_text,answer_text,points,sort_order FROM cds_olympia_questions WHERE week_id=? ORDER BY sort_order,id');$rows->execute([$sourceId]);$ins=$db->prepare('INSERT INTO cds_olympia_questions(id,week_id,stage_name,question_text,answer_text,points,sort_order) VALUES(?,?,?,?,?,?,?)');foreach($rows as $row){$ins->execute([olympia_uid('oq'),$targetId,$row['stage_name'],$row['question_text'],$row['answer_text'],$row['points'],$row['sort_order']]);$counts['questions']++;}}
    return $counts;
}
