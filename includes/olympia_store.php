...
function olympia_ranking_students(array $weekIds): array {
    ...
    $s=olympia_db()->prepare("SELECT student_id,student_name,class_id,class_name,SUM(points) points,COUNT(*) correct,COUNT(DISTINCT week_id) weeks FROM cds_olympia_scores WHERE week_id IN ($marks) GROUP BY student_id,student_name,class_id,class_name ORDER BY points DESC,correct DESC,student_name ASC");
    $s->execute($weekIds);return $s->fetchAll();
}
...
function olympia_normalize_grade_scope($value): string {
    if(is_array($value))$parts=$value;else{$text=trim((string)$value);if($text===''||in_array(mb_strtolower($text,'UTF-8'),['all','tất cả','tat ca'],true))return 'all';$parts=preg_split('/\s*[,;]\s*/u',$text);}
    $grades=[];foreach((array)$parts as $part){$grade=(int)trim((string)$part);if($grade>=1&&$grade<=12)$grades[]=$grade;}$grades=array_values(array_unique($grades));sort($grades,SORT_NUMERIC);return $grades?implode(',',$grades):'all';
}

function olympia_class_grade(array $class): string {
    $name=trim((string)($class['name']??$class['class_name']??''));return preg_match('/(?:^|\D)(1[0-2]|[1-9])(?:\D|$)/u',$name,$match)?(string)(int)$match[1]:'';
}

function olympia_copy_week_content(string $sourceId,string $targetId,bool $copyClasses,bool $copyQuestions): array {
...
function olympia_ensure_schema(): void {
    ...
    $sql=[
        ...
        "CREATE TABLE IF NOT EXISTS cds_olympia_questions (id VARCHAR(40) NOT NULL,week_id VARCHAR(40) NOT NULL,grade_scope VARCHAR(40) NOT NULL DEFAULT 'all',stage_name VARCHAR(60) NOT NULL DEFAULT 'Khởi động',question_text TEXT NOT NULL,answer_text TEXT NOT NULL,points INT NOT NULL DEFAULT 10,sort_order INT NOT NULL DEFAULT 0,created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,PRIMARY KEY(id),KEY idx_olympia_q_week(week_id,sort_order),CONSTRAINT fk_olympia_q_week FOREIGN KEY(week_id) REFERENCES cds_olympia_weeks(id) ON DELETE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci",
        ...
        ,"CREATE TABLE IF NOT EXISTS cds_olympia_play_history (id VARCHAR(64) NOT NULL,week_id VARCHAR(40) NOT NULL,class_id VARCHAR(100) NOT NULL,class_name VARCHAR(100) NOT NULL DEFAULT '',status VARCHAR(32) NOT NULL DEFAULT 'welcome',last_state LONGTEXT NOT NULL,controlled_by VARCHAR(255) NOT NULL DEFAULT '',started_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,finished_at DATETIME NULL,updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,PRIMARY KEY(id),KEY idx_olympia_history_week_class(week_id,class_id,updated_at),CONSTRAINT fk_olympia_history_week FOREIGN KEY(week_id) REFERENCES cds_olympia_weeks(id) ON DELETE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci"
    ];
    foreach($sql as $statement)$db->exec($statement);
    $column=$db->query("SHOW COLUMNS FROM cds_olympia_questions LIKE 'grade_scope'")->fetch();if(!$column)$db->exec("ALTER TABLE cds_olympia_questions ADD COLUMN grade_scope VARCHAR(40) NOT NULL DEFAULT 'all' AFTER week_id");
}
...
function olympia_questions(string $week,string $grade=''): array { if($grade===''){$s=olympia_db()->prepare('SELECT * FROM cds_olympia_questions WHERE week_id=? ORDER BY sort_order,id');$s->execute([$week]);}else{$s=olympia_db()->prepare("SELECT * FROM cds_olympia_questions WHERE week_id=? AND (grade_scope='all' OR FIND_IN_SET(?,REPLACE(grade_scope,' ',''))>0) ORDER BY sort_order,id");$s->execute([$week,$grade]);}return $s->fetchAll(); }
...
function olympia_play_locked(string $weekId,string $classId): bool {$s=olympia_db()->prepare('SELECT is_locked FROM cds_olympia_play_controls WHERE week_id=? AND class_id=?');$s->execute([$weekId,$classId]);return (int)$s->fetchColumn()===1;}
function olympia_play_max_attempts(string $weekId,string $classId): int {$s=olympia_db()->prepare('SELECT max_attempts FROM cds_olympia_play_controls WHERE week_id=? AND class_id=?');$s->execute([$weekId,$classId]);$value=$s->fetchColumn();return $value===false?1:max(1,(int)$value);}
function olympia_play_start(string $weekId,string $classId,string $actor): array {if(olympia_play_locked($weekId,$classId))throw new RuntimeException('Quản trị đang khóa lượt chơi của lớp này.');$db=olympia_db();$db->beginTransaction();try{$s=$db->prepare("SELECT * FROM cds_olympia_play_runs WHERE week_id=? AND class_id=? AND status='running' ORDER BY id DESC LIMIT 1 FOR UPDATE");$s->execute([$weekId,$classId]);$row=$s->fetch();if(!$row){$n=$db->prepare('SELECT COALESCE(MAX(attempt_no),0)+1 FROM cds_olympia_play_runs WHERE week_id=? AND class_id=?');$n->execute([$weekId,$classId]);$attempt=(int)$n->fetchColumn();$max=olympia_play_max_attempts($weekId,$classId);if($attempt>$max)throw new RuntimeException('Lớp đã dùng đủ '.$max.' lượt chơi trong tuần này. Vui lòng liên hệ quản trị để reset hoặc tăng số lượt.');$class=olympia_class_map()[$classId]??[];$i=$db->prepare("INSERT INTO cds_olympia_play_runs(week_id,class_id,class_name,attempt_no,status,started_by) VALUES(?,?,?,?,'running',?)");$i->execute([$weekId,$classId,(string)($class['name']??''),$attempt,$actor]);$id=(int)$db->lastInsertId();$s=$db->prepare('SELECT * FROM cds_olympia_play_runs WHERE id=?');$s->execute([$id]);$row=$s->fetch();}$db->commit();return $row?:[];}catch(Throwable $e){if($db->inTransaction())$db->rollBack();throw $e;}}
