...
$lockStatus = noitru_duty_report_lock_status($reportDate, $report);
$isReportLocked = !empty($lockStatus['locked']);
$lockSettings = noitru_duty_report_lock_settings();
$canManageReportLock = allowed_classes() === null && can_edit_perm('nt.lichtruc');
...
<div class="duty-report-toolbar">
  ...
  <form method="get" class="d-flex gap-2 align-items-end"><input type="hidden" name="tab" value="duty_report"><div><label class="form-label small mb-1">Ngày trực</label><input type="date" name="date" class="form-control" value="<?= e($reportDate) ?>"></div><button class="btn btn-duty-view"><i class="bi bi-search"></i> Xem</button></form>
</div>

<div class="duty-report-admin">
  <span class="duty-lock-state <?= $isReportLocked?'text-danger':'text-success' ?>"><i class="bi <?= $isReportLocked?'bi-lock-fill':'bi-unlock' ?>"></i> <?= $isReportLocked ? 'Biên bản đã khóa'.($lockStatus['source']==='automatic'?' tự động':' thủ công') : 'Biên bản đang mở' ?></span>
  <?php if($canManageReportLock): ?><form method="post"><input type="hidden" name="action" value="duty_report_lock"><input type="hidden" name="date" value="<?=e($reportDate)?>"><input type="hidden" name="mode" value="<?=$isReportLocked?'unlock':'lock'?>"><button class="btn <?=$isReportLocked?'btn-outline-success':'btn-outline-danger'?>" type="submit"><i class="bi <?=$isReportLocked?'bi-unlock':'bi-lock'?>"></i> <?=$isReportLocked?'Mở khóa ngày này':'Khóa ngày này'?></button></form>
  <details><summary class="btn btn-outline-secondary"><i class="bi bi-clock-history"></i> Lịch khóa tự động</summary><form method="post" class="card card-body mt-2"><input type="hidden" name="action" value="duty_report_lock_settings"><input type="hidden" name="date" value="<?=e($reportDate)?>"><label class="form-check mb-2"><input class="form-check-input" type="checkbox" name="auto_lock_enabled" <?=!empty($lockSettings['auto_lock_enabled'])?'checked':''?>> Tự động khóa</label><label class="form-label">Sau số ngày</label><input class="form-control mb-2" type="number" min="1" max="30" name="auto_lock_after_days" value="<?=(int)$lockSettings['auto_lock_after_days']?>"><label class="form-label">Giờ khóa</label><input class="form-control mb-2" type="time" name="lock_time" value="<?=e((string)$lockSettings['lock_time'])?>"><button class="btn btn-primary">Lưu lịch khóa</button></form></details><?php endif; ?>
  <details><summary class="btn btn-outline-primary"><i class="bi bi-file-zip"></i> Xuất ZIP nhiều ngày</summary><form method="get" action="<?=e(BASE_URL.'noitru_duty_reports_zip.php')?>" class="card card-body mt-2"><label class="form-label">Từ ngày</label><input class="form-control mb-2" type="date" name="from" required value="<?=e($reportDate)?>"><label class="form-label">Đến ngày</label><input class="form-control mb-2" type="date" name="to" required value="<?=e($reportDate)?>"><button class="btn btn-primary"><i class="bi bi-download"></i> Tải ZIP</button></form></details>
</div>
...
<form method="post" id="dutyReportForm" data-csrf="<?=e(cds_drive_csrf_token())?>" data-date="<?=e($reportDate)?>" data-drive-endpoint="<?=e(BASE_URL.'noitru_duty_drive.php')?>">
  ...
  <div class="duty-report-actions"><button class="btn btn-duty-preview" type="button" id="toggleDutyPreview"><i class="bi bi-eye"></i> Xem trước</button><button class="btn btn-duty-save" id="saveDutyReport" <?= !$canEditCurrent?'disabled':'' ?>><i class="bi bi-floppy"></i> Lưu biên bản</button><button class="btn btn-duty-print" type="button" onclick="printDutyReport()"><i class="bi bi-printer"></i> In / Xuất PDF</button></div>
...
(function(){
  ...
  async function createPdfBlob(){
    ...
    const attendance=clone.querySelector('.report-attendance');
    if(attendance){
      const pdfTable=document.createElement('div');
      pdfTable.className='pdf-attendance-table';
      attendance.querySelectorAll('thead tr,tbody tr').forEach(function(source){
        const row=document.createElement('div');
        row.className='pdf-attendance-row'+(source.closest('thead')?' pdf-attendance-head':'');
        Array.from(source.children).forEach(function(sourceCell){
          const cell=document.createElement('div');
          cell.className='pdf-attendance-cell '+(sourceCell.className||'');
          cell.innerHTML=sourceCell.innerHTML;
          row.appendChild(cell);
        });
        pdfTable.appendChild(row);
      });
      attendance.replaceWith(pdfTable);
    }
    clone.querySelectorAll('.pdf-attendance-row,.report-signatures').forEach(function(el){
    ...
    try{
      return await window.html2pdf().set({
        ...
        pagebreak:{mode:['css','legacy'],avoid:['.pdf-attendance-row','.report-signatures','.pdf-content-block']}
  ...
  form?.addEventListener('submit',async function(event){
    ...
    saveBtn.innerHTML='<span class="spinner-border spinner-border-sm me-1"></span> Đang lưu dữ liệu…';
    if(saveStatus){saveStatus.className='drive-save-note text-primary';saveStatus.textContent='Đang lưu nội dung biên bản vào hệ thống…';}
    try{
