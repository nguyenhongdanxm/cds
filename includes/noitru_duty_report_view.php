...
foreach (noitru_att_all() as $row) {
    ...
    if (in_array($status, ['absent','late','excused'], true)) {
        ...
        $detail = ['name'=>$student['name'] ?? ($row['student_name'] ?? 'Không rõ'),'class'=>$student['class_name'] ?? ($row['class_name'] ?? ''),'status'=>$status,'reason'=>trim((string)($row['reason'] ?? ''))];
        if (in_array($status,['absent','excused'],true)) {
            $className=trim((string)$detail['class'])?:'(Chưa rõ lớp)';
            $absentByShiftClass[$shift][$className][]=$detail;
        }
    }
}
...
<form method="post" id="dutyReportForm">
  ...
  <div class="duty-report-preview-wrap"><article class="duty-report-paper">
    ...
    <div class="report-section"><div class="report-section-title">3. Nội dung</div><div class="report-subtitle">3.1. Dữ liệu các lần điểm danh</div>
      <?php if($attendanceByShift): ?><table class="report-attendance"><thead><tr><th>Điểm danh</th><th>Sỹ số<br><small>(Có mặt/Tổng)</small></th><th>Vắng (liệt kê học sinh)</th></tr></thead><tbody><?php foreach($attendanceByShift as $shift=>$counts): $total=array_sum($counts);$presentTotal=(int)$counts['present']+(int)$counts['late'];$absentTotal=(int)$counts['absent']+(int)$counts['excused'];$shiftAbsent=$absentByShiftClass[$shift]??[]; ?><tr><td><?= e($shiftLabels[$shift]??$shift) ?></td><td><strong><?= $presentTotal ?>/<?= (int)$total ?></strong></td><td class="attendance-absent"><?php if($shiftAbsent): foreach($shiftAbsent as $className=>$items): ?><div class="attendance-class"><strong><?=e($className)?>:</strong> <?php foreach($items as $index=>$item): ?><?= $index?', ':'' ?><?=e($item['name'])?> (<?=$item['status']==='excused'?'P':'KP'?>)<?php endforeach; ?></div><?php endforeach; elseif($absentTotal): ?>Có <?=$absentTotal?> học sinh vắng; chưa có danh sách chi tiết.<?php else: ?>Không có học sinh vắng.<?php endif; ?></td></tr><?php endforeach; ?></tbody></table><?php else: ?><div class="report-empty">Chưa có dữ liệu điểm danh trong ngày.</div><?php endif; ?>
