...
$dutyMonthStats = [];
foreach ($dutyTeacherMap as $teacherId=>$teacherName) $dutyMonthStats[$teacherId] = 0;
foreach ($dutyMonthRows as $row) {
    $teacherId = (string)($row['teacher_id'] ?? '');
...
@media(max-width:991.98px){.duty-calendar{grid-template-columns:repeat(4,minmax(0,1fr))}.duty-calendar-blank{display:none}.duty-stat-cards{grid-template-columns:1fr 1fr}.duty-settings-grid{grid-template-columns:1fr}.duty-tabs{overflow-x:auto;display:flex}.duty-tabs a{flex:0 0 auto;min-width:128px}}
...
<?php elseif ($dutySection === 'assign'): ?>
  <section class="duty-panel">
    ...
    <div class="duty-panel-body">
      ...
      <form method="post" id="dutyToggleForm"><input type="hidden" name="action" value="duty_toggle"><input type="hidden" name="month" value="<?= e($dutyMonth) ?>"><input type="hidden" name="section" value="assign"><input type="hidden" name="teacher_id" id="dutyToggleTeacher"><input type="hidden" name="date" id="dutyToggleDate"></form>
      <div class="duty-matrix-wrap"><table class="duty-matrix"><thead><tr><th class="teacher-col">Giáo viên</th><th class="count-col">Lần</th><?php for($day=1;$day<=$dutyDays;$day++):$date=$dutyMonth.'-'.str_pad((string)$day,2,'0',STR_PAD_LEFT);$weekday=(int)date('N',strtotime($date));?><th class="<?= $weekday>=6?'duty-weekend-col':'' ?>"><small><?= e($dutyShortWeekdays[$weekday-1]) ?></small><br><?= $day ?></th><?php endfor;?></tr></thead><tbody>
      <?php foreach ($dutyTeacherMap as $teacherId=>$teacherName): ?><tr><td class="teacher-col"><i class="bi bi-person-circle text-info me-1"></i><strong><?= e($teacherName) ?></strong></td><td class="count-col"><span class="badge bg-light text-dark border"><?= (int)($dutyMonthStats[$teacherId]??0) ?>/<?= (int)$dutySettings['max_per_month'] ?></span></td><?php for($day=1;$day<=$dutyDays;$day++):$date=$dutyMonth.'-'.str_pad((string)$day,2,'0',STR_PAD_LEFT);$selected=isset($dutyByTeacher[$teacherId][$date]);$weekday=(int)date('N',strtotime($date));?><td class="<?= $weekday>=6?'duty-weekend-col':'' ?>"><button type="button" class="duty-matrix-day <?= $selected?'selected':'' ?>" onclick="toggleDuty('<?= e($teacherId) ?>','<?= e($date) ?>')" aria-label="<?= $selected?'Bỏ':'Thêm' ?> lịch trực <?= e($teacherName) ?> ngày <?= e(date('d/m/Y',strtotime($date))) ?>"><i class="bi bi-check"></i></button></td><?php endfor;?></tr><?php endforeach; ?>
      </tbody></table></div>
    </div>
  </section>
  <script>function toggleDuty(teacherId,date){document.getElementById('dutyToggleTeacher').value=teacherId;document.getElementById('dutyToggleDate').value=date;document.getElementById('dutyToggleForm').submit()}</script>

<?php elseif ($dutySection === 'manage'): ?>
  <section class="duty-panel"><div class="duty-panel-head"><div><h6><i class="bi bi-shield-check text-warning"></i> Phân công quản lý trực</h6><div class="duty-help">Có thể chọn nhiều quản lý cho cùng một ngày. Giữ Ctrl/Cmd để chọn nhiều người.</div></div></div><div class="table-responsive"><table class="table duty-manager-table mb-0"><thead><tr><th>Ngày</th><th>Thứ</th><th>Quản lý trực</th><th>Ghi chú</th><th></th></tr></thead><tbody>
