...
<?php elseif ($dutySection === 'requests'): ?>
  <?php
  $dutyNormalizeName=static function($value){$value=preg_replace('/\s+/u',' ',trim((string)$value));return function_exists('mb_strtolower')?mb_strtolower($value,'UTF-8'):strtolower($value);};
  $dutyCurrentNameKey=$dutyNormalizeName($dutyCurrentTeacherName);
  $dutyIsMine=static fn($row)=>(string)($row['teacher_id']??'')===$dutyCurrentTeacherId||($dutyCurrentNameKey!==''&&$dutyNormalizeName($row['teacher_name']??'')===$dutyCurrentNameKey);
  $myFutureDuties=array_values(array_filter($dutyMonthRows,fn($row)=>$dutyIsMine($row)&&($row['date']??'')>=$dutyToday));
  $otherFutureDuties=array_values(array_filter($dutyMonthRows,fn($row)=>!$dutyIsMine($row)&&($row['date']??'')>=$dutyToday));
  ?>
  <div class="duty-request-grid mb-3">
    <section class="duty-panel"><div class="duty-panel-head"><div><h6><i class="bi bi-arrow-left-right text-info"></i> Đăng ký đổi lịch trực</h6><div class="duty-help">Chọn một lịch của mình và một lịch muốn đổi. Lịch chính thức chỉ thay đổi sau khi được duyệt.</div></div></div><div class="duty-panel-body" id="dutySwapFormBody" style="display:block;min-height:220px">
  ...
  <script>
  (function(){
    var body=document.getElementById('dutySwapFormBody');
    if(!body||body.querySelector('select[name="source_duty_id"]'))return;
    ...
    fill(form.elements.source_duty_id,mine,mine.length?'Chọn ngày cần đổi':'Không có lịch trực phù hợp trong tháng này');
    fill(form.elements.target_duty_id,others,others.length?'Chọn lịch muốn đổi':'Không có lịch của giáo viên khác để đổi');
    if(!mine.length||!others.length){form.elements.reason.disabled=true;form.querySelector('button').disabled=true;if(!body.querySelector('.alert')){var notice=document.createElement('div');notice.className='alert alert-info';notice.textContent='Không có lịch trực phù hợp trong tháng đang xem. Hãy chuyển tháng hoặc kiểm tra tài khoản giáo viên.';body.appendChild(notice)}}
    body.appendChild(form);
  })();
  </script>
  <section class="duty-panel"><div class="duty-panel-head"><div><h6><?= $canManageDuty?'Yêu cầu đổi lịch cần xử lý':'Yêu cầu liên quan đến tôi' ?></h6><div class="duty-help"><?= $canManageDuty?'Duyệt, từ chối hoặc xóa yêu cầu; thao tác duyệt sẽ cập nhật lịch trực ngay.':'Theo dõi trạng thái các yêu cầu đã gửi hoặc liên quan.' ?></div></div></div><div class="table-responsive"><table class="table align-middle mb-0"><thead><tr><th>Ngày gửi</th><th>Người đề nghị</th><th>Đổi lịch</th><th>Lý do</th><th>Trạng thái</th><?php if($canManageDuty):?><th>Xử lý</th><?php endif;?></tr></thead><tbody><?php foreach($dutySwapRequests as $request):?><tr><td><?=e(!empty($request['created_at'])?date('d/m/Y H:i',strtotime($request['created_at'])):'')?></td><td><strong><?=e($request['requester_name']??'')?></strong></td><td><?=e($request['source_teacher_name']??'')?> · <?=e(date('d/m/Y',strtotime($request['source_date']??'')))?><br><i class="bi bi-arrow-down-up text-info"></i> <?=e($request['target_teacher_name']??'')?> · <?=e(date('d/m/Y',strtotime($request['target_date']??'')))?></td><td><?=e($request['reason']??'')?><?php if(!empty($request['decision_note'])):?><div class="text-muted small">Phản hồi: <?=e($request['decision_note'])?></div><?php endif;?></td><td><span class="duty-status <?=e($request['status']??'pending')?>"><?=e(['pending'=>'Chờ duyệt','approved'=>'Đã duyệt','rejected'=>'Từ chối'][$request['status']??'pending']??'Chờ duyệt')?></span></td><?php if($canManageDuty):?><td><?php if(($request['status']??'')==='pending'):?><form method="post" class="d-flex gap-1 flex-wrap mb-1"><input type="hidden" name="action" value="duty_swap_request_decide"><input type="hidden" name="request_id" value="<?=e($request['id']??'')?>"><input type="hidden" name="month" value="<?=e($dutyMonth)?>"><input type="text" name="decision_note" class="form-control form-control-sm" placeholder="Ghi chú"><button name="decision" value="approved" class="btn btn-sm btn-success" onclick="return confirm('Duyệt và cập nhật lịch trực?')">Duyệt</button><button name="decision" value="rejected" class="btn btn-sm btn-outline-danger">Từ chối</button></form><?php else:?><small class="d-block mb-1"><?=e($request['decision_by']??'')?><?=!empty($request['decided_at'])?' · '.e(date('d/m/Y H:i',strtotime($request['decided_at']))):''?></small><?php endif;?><form method="post" onsubmit="return confirm('Xóa vĩnh viễn yêu cầu đổi lịch trực này?')"><input type="hidden" name="action" value="duty_swap_request_delete"><input type="hidden" name="request_id" value="<?=e($request['id']??'')?>"><input type="hidden" name="month" value="<?=e($dutyMonth)?>"><button class="btn btn-sm btn-outline-danger"><i class="bi bi-trash"></i> Xóa</button></form></td><?php endif;?></tr><?php endforeach;?><?php if(!$dutySwapRequests):?><tr><td colspan="<?= $canManageDuty?6:5 ?>" class="text-center text-muted py-4">Chưa có yêu cầu đổi lịch.</td></tr><?php endif;?></tbody></table></div></section>
...
<?php elseif ($dutySection === 'manage'): ?>
  ...
  <?php for($day=1;$day<=$dutyDays;$day++):$date=$dutyMonth.'-'.str_pad((string)$day,2,'0',STR_PAD_LEFT);$weekday=(int)date('N',strtotime($date));$manager=$dutyManagerByDate[$date]??[];$selectedIds=$manager['teacher_ids']??[];$hasManager=!empty($selectedIds)||trim((string)($manager['note']??''))!== ''; ?><tr class="<?= $weekday>=6?'weekend':'' ?>"><td><strong><?= e(date('d/m',strtotime($date))) ?></strong></td><td><?= e($dutyWeekdayNames[$weekday-1]) ?></td><td colspan="3"><form method="post"><input type="hidden" name="action" value="duty_manager_save"><input type="hidden" name="month" value="<?= e($dutyMonth) ?>"><input type="hidden" name="section" value="manage"><input type="hidden" name="date" value="<?= e($date) ?>"><select name="teacher_ids[]" class="form-select form-select-sm" multiple size="2"><?php foreach($dutyTeacherMap as $id=>$name):?><option value="<?= e($id) ?>" <?= in_array($id,$selectedIds,true)?'selected':'' ?>><?= e($name) ?></option><?php endforeach;?></select><input type="text" name="note" class="form-control form-control-sm manager-note" placeholder="Ghi chú" value="<?= e($manager['note']??'') ?>"><button class="btn btn-sm btn-outline-info">Lưu</button><?php if($isDutyAdmin&&$hasManager):?><button class="btn btn-sm btn-outline-danger" name="clear_manager" value="1" onclick="return confirm('Xóa toàn bộ quản lý trực ngày <?=e(date('d/m/Y',strtotime($date)))?>?')" title="Xóa nhanh quản lý trực"><i class="bi bi-trash"></i> Xóa</button><?php endif;?></form></td></tr><?php endfor; ?>
