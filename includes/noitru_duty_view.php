  ...
  <section class="duty-panel">
    ...
    <div class="duty-panel-body">
      ...
      <div class="duty-tools-grid">
        <div class="duty-tool-box"><h6><i class="bi bi-person-plus-fill text-success"></i> Phân công bằng tay</h6><form method="post" class="duty-tool-form"><input type="hidden" name="action" value="duty_save"><input type="hidden" name="month" value="<?= e($dutyMonth) ?>"><select name="teacher_id" class="form-select form-select-sm" required><option value="">Chọn người trực</option><?php foreach($dutyRosterMap as $id=>$name):?><option value="<?= e($id) ?>"><?= e($name) ?></option><?php endforeach;?></select><input type="date" name="date" class="form-control form-control-sm" value="<?= e($dutyToday >= $dutyMonthStart && $dutyToday <= $dutyMonth.'-'.$dutyDays ? $dutyToday : $dutyMonthStart) ?>" min="<?= e($dutyMonthStart) ?>" max="<?= e($dutyMonth.'-'.$dutyDays) ?>" required><input type="text" name="note" class="form-control form-control-sm" placeholder="Ghi chú (không bắt buộc)"><button class="btn btn-sm btn-success text-white"><i class="bi bi-check2-circle"></i> Thêm lượt trực</button></form></div>
      ...
      <?php if($dutySuggestionIds):?><div class="duty-help mb-2"><i class="bi bi-lightbulb text-warning"></i> Gợi ý bù lượt tháng này (ưu tiên người trực ít ở tháng <?= e(date('m/Y',strtotime($dutyPrevMonth.'-01'))) ?>):</div><div class="duty-suggestions"><?php foreach(array_slice($dutySuggestionIds,0,8) as $teacherId):?><span class="duty-suggestion"><strong><?= e($dutyRosterMap[$teacherId]??'') ?></strong>Tháng trước: <?= (int)($dutyPreviousStats[$teacherId]??0) ?> lượt · CN: <?= (int)($dutyPreviousSundayStats[$teacherId]??0) ?></span><?php endforeach;?></div><?php endif;?>
      <form method="post" id="dutyToggleForm"><input type="hidden" name="action" value="duty_toggle"><input type="hidden" name="month" value="<?= e($dutyMonth) ?>"><input type="hidden" name="section" value="assign"><input type="hidden" name="teacher_id" id="dutyToggleTeacher"><input type="hidden" name="date" id="dutyToggleDate"><input type="hidden" name="force" id="dutyToggleForce" value="0"></form>
      <?php if(!$dutyRosterMap):?><div class="alert alert-warning">Danh sách trực đang trống. Hãy thêm người tại tab <strong>Cài đặt</strong>.</div><?php else:?><div class="duty-matrix-wrap"><table class="duty-matrix"><thead><tr><th class="teacher-col">Giáo viên</th><th class="count-col">Lần</th><th class="count-col">CN</th><?php for($day=1;$day<=$dutyDays;$day++):$date=$dutyMonth.'-'.str_pad((string)$day,2,'0',STR_PAD_LEFT);$weekday=(int)date('N',strtotime($date));?><th class="<?= $weekday>=6?'duty-weekend-col':'' ?>"><small><?= e($dutyShortWeekdays[$weekday-1]) ?></small><br><?= $day ?></th><?php endfor;?></tr></thead><tbody>
      <?php foreach ($dutyRosterMap as $teacherId=>$teacherName):$personalLimit=($dutyRosterLimits[$teacherId]??0)>0?$dutyRosterLimits[$teacherId]:(int)$dutySettings['max_per_month']; ?><tr><td class="teacher-col"><i class="bi bi-person-circle text-info me-1"></i><strong><?= e($teacherName) ?></strong></td><td class="count-col"><span class="badge bg-light text-dark border"><?= (int)($dutyMonthStats[$teacherId]??0) ?>/<?= (int)$personalLimit ?></span></td><td class="count-col"><span class="badge bg-light text-warning border"><?= (int)($dutySundayStats[$teacherId]??0) ?></span></td><?php for($day=1;$day<=$dutyDays;$day++):$date=$dutyMonth.'-'.str_pad((string)$day,2,'0',STR_PAD_LEFT);$selected=isset($dutyByTeacher[$teacherId][$date]);$weekday=(int)date('N',strtotime($date));$dayCount=count($dutyByDate[$date]??[]);?><td class="<?= $weekday>=6?'duty-weekend-col':'' ?>"><button type="button" class="duty-matrix-day <?= $selected?'selected':'' ?>" onclick="toggleDuty('<?= e($teacherId) ?>','<?= e($date) ?>',<?= $dayCount ?>,<?= (int)$dutySettings['people_per_day'] ?>,<?= $selected?'true':'false' ?>)" aria-label="<?= $selected?'Bỏ':'Thêm' ?> lịch trực <?= e($teacherName) ?> ngày <?= e(date('d/m/Y',strtotime($date))) ?>"><i class="bi bi-check"></i></button></td><?php endfor;?></tr><?php endforeach; ?>
      </tbody></table></div>
      <?php endif;?>
    </div>
  </section>
  <script>function toggleDuty(teacherId,date,count,limit,selected){var force=document.getElementById('dutyToggleForce');force.value='0';if(!selected&&count>=limit){if(!confirm('Ngày này đã đủ '+limit+' người theo cài đặt. Bạn vẫn muốn phân công thêm?'))return;force.value='1'}document.getElementById('dutyToggleTeacher').value=teacherId;document.getElementById('dutyToggleDate').value=date;document.getElementById('dutyToggleForm').submit()}</script>
