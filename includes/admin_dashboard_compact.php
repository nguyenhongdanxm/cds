...
ob_start(static function (string $html) use ($latestDocumentsJson, $vanbanUrl, $checkUrl): string {
    $script = <<<'HTML'
<style id="cds-dashboard-compact-v2">
/* Không cho thông báo/công việc phá vỡ khối trên màn hình hẹp. */
...
(function(){
 ...
 /* Truy cập nhanh: đúng 5 mục theo yêu cầu. */
 ...
 if(quick){
   ...
   const wanted=[find('điểm danh'),find('báo ăn'),find('thời khóa biểu'),find('chuyên môn')].filter(Boolean);
   ...
   wanted.push(check);quick.replaceChildren(...wanted);quick.classList.add('cds-five-actions');
 }
 /* Chèn 5 văn bản mới nhất ngay cạnh các khối nghiệp vụ. */
 ...
 /* Bảo đảm Văn bản xuất hiện trong Hệ sinh thái CDS nếu người dùng có trang Văn bản nhưng module đang bị lọc thiếu. */
