...
<div class="table-wrap desktop-table">
  <table class="table">
    ...
    <?php else: foreach ($shown as $row): $fileUrl=vb_file_url((string)($row['file_path']??'')); ?>
      <tr>
        <td><strong><?=e($row['symbol']??'')?></strong><div class="sub"><?=e($row['field']??'')?> <?=!empty($row['featured'])?'· ⭐ Nổi bật':''?></div></td>
...
<?php else:foreach($shown as $row):$fileUrl=vb_file_url((string)($row['file_path']??''));?><article class="card doc-card">
  ...
  <div class="sub"><?=e($row['symbol']??'')?> · <?=vb_fmt_date((string)($row['issued_date']??''))?></div><div class="meta"><span class="pill"><?=e($row['type']??'')?></span><span class="pill"><?=e($row['field']??'')?></span></div><div class="sub"><?=e($row['issuer']??'')?></div>
