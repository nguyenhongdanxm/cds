...
function nt_boarders_table(array $list) {
    ...
    foreach ($list as $i => $s) {
        ...
        echo '<tr><td>'.($i+1).'</td>';
        $studentId=(string)($s['id']??'');$photoUrl=BASE_URL.'student_photo.php?id='.rawurlencode($studentId).'&v='.rawurlencode((string)($s['photo_updated_at']??''));
        echo '<td><div class="nt-student-identity"><img class="nt-student-thumb" src="'.e($photoUrl).'" alt="" loading="lazy" onerror="this.classList.add(\'is-missing\')"><div><button type="button" class="nt-student-name" data-bs-toggle="modal" data-bs-target="#ntStudentDetailModal" data-student="'.e($studentJson).'">'.e($s['name'] ?? '').'</button>';
        if (!empty($s['code'])) echo '<div class="small text-muted">'.e($s['code']).'</div>';
        echo '</div></div></td><td>'.e(($s['class_name'] ?? '') ?: '—').'</td>';
...
?>
<style>
.nt-room-role-box{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:.55rem;margin:.65rem 0 1rem}.nt-room-role{padding:.65rem .75rem;border:1px solid #e5e7eb;border-radius:12px;background:#fff}.nt-room-role small{display:block;color:#64748b;font-size:.72rem}.nt-room-role strong{display:block;margin-top:.15rem;font-size:.9rem;overflow-wrap:anywhere}.nt-room-card-meta{margin-top:.5rem;padding-top:.45rem;border-top:1px dashed #e5e7eb;font-size:.72rem;line-height:1.45;color:#64748b}.nt-room-card-meta b{color:#334155;font-weight:600}.nt-student-identity{display:flex;align-items:center;gap:.55rem;min-width:170px}.nt-student-thumb{width:38px;height:50px;flex:0 0 38px;object-fit:cover;border-radius:7px;border:1px solid #d8e2ec;background:#edf2f7}.nt-student-thumb.is-missing{visibility:hidden;width:0;flex-basis:0;border:0;margin-right:-.55rem}.nt-profile-avatar img{width:100%;height:100%;object-fit:cover;display:block}.nt-profile-avatar.has-photo i{display:none}@media(max-width:700px){.nt-room-role-box{grid-template-columns:1fr}.nt-room-card-meta{font-size:.68rem}.nt-student-thumb{width:32px;height:43px;flex-basis:32px}}
</style>
...
<div class="modal fade nt-profile-modal" id="ntStudentDetailModal" tabindex="-1" aria-labelledby="ntStudentDetailTitle" aria-hidden="true"><div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg"><div class="modal-content"><div class="nt-profile-head"><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Đóng"></button><div class="nt-profile-person"><div class="nt-profile-avatar"><img data-student-photo alt="Ảnh thẻ học sinh" hidden><i class="bi bi-person-fill"></i></div><div><h5 id="ntStudentDetailTitle" data-student-field="name">Thông tin học sinh</h5><p><span data-student-field="code">—</span> · Lớp <span data-student-field="class_name">—</span></p></div></div></div><div class="nt-profile-body">
...
<script>document.getElementById('ntStudentDetailModal')?.addEventListener('show.bs.modal',function(event){var trigger=event.relatedTarget,student={};try{student=JSON.parse(trigger?.dataset.student||'{}')}catch(e){}this.querySelectorAll('[data-student-field]').forEach(function(field){var key=field.dataset.studentField,value=String(student[key]??'').trim();if(key==='dob'&&/^\d{4}-\d{2}-\d{2}$/.test(value)){var p=value.split('-');value=p[2]+'/'+p[1]+'/'+p[0]}field.textContent=value||'—'});var img=this.querySelector('[data-student-photo]'),avatar=img?.closest('.nt-profile-avatar'),id=String(student.id||'').trim();if(img&&id){img.hidden=false;avatar?.classList.add('has-photo');img.src=<?=json_encode(BASE_URL.'student_photo.php?id=')?>+encodeURIComponent(id)+'&v='+encodeURIComponent(String(student.photo_updated_at||Date.now()))}});document.querySelector('[data-student-photo]')?.addEventListener('error',function(){this.hidden=true;this.closest('.nt-profile-avatar')?.classList.remove('has-photo')});</script>
