(function(){
  ...
  function byClass(items,showType){if(!items.length)return '<span class="report-empty">Không có</span>';var groups={};items.forEach(function(r){var c=(r.class||'(Chưa rõ lớp)').trim()||'(Chưa rõ lớp)';(groups[c]||(groups[c]=[])).push(r);});return Object.keys(groups).sort(function(a,b){return a.localeCompare(b,'vi',{numeric:true});}).map(function(c){return '<div class="attendance-class"><strong>'+esc(c)+':</strong> '+groups[c].map(function(r){return esc(r.name)+(showType?' ('+esc(r.excuse||'KP')+')':'')+(r.reason?' – '+esc(r.reason):'');}).join(', ')+'</div>';}).join('');}
  function block(label,items,showType){return '<div class="nt-duty-absence-group"><div class="nt-duty-absence-label">'+esc(label)+'</div><div>'+byClass(items,showType)+'</div></div>';}
  var style=document.createElement('style');style.textContent='.nt-duty-absence-group{display:grid;grid-template-columns:48mm 1fr;gap:2mm;padding:1.1mm 0;border-bottom:1px dotted #bbb}.nt-duty-absence-group:last-child{border-bottom:0}.nt-duty-absence-label{font-style:normal;font-weight:700}.nt-duty-absence-group .attendance-class{margin:0 0 .6mm}.nt-duty-absence-group .attendance-class:last-child{margin-bottom:0}@media(max-width:900px){.nt-duty-absence-group{grid-template-columns:1fr}}';document.head.appendChild(style);
