(function(){
  ...
  function block(label,items){return '<div class="nt-duty-absence-group"><div class="nt-duty-absence-label">'+esc(label)+'</div><div>'+byClass(items)+'</div></div>';}
  ...
  fetch(endpoint.toString(),{credentials:'same-origin'}).then(function(r){return r.json();}).then(function(data){if(!data||!data.ok||!Array.isArray(data.rows))return;var trs=[].slice.call(document.querySelectorAll('.report-attendance tbody tr'));trs.forEach(function(tr){var tds=tr.querySelectorAll('td');if(tds.length<3)return;var label=(tds[0].textContent||'').trim(),rows=data.rows.filter(function(r){return norm(r.shift_label||r.shift)===norm(label);});if(!rows.length&&trs.length===1)rows=data.rows;if(!rows.length)return;var p=[],m=[],k=[];rows.forEach(function(r){var code=String(r.absence_type||r.excuse||'');if(code==='P_SAU_AN'||r.meal_after_registration)m.push(r);else if(code==='P')p.push(r);else k.push(r);});tds[2].innerHTML=block('Có phép',p)+block('Có phép sau thời gian đăng ký bữa ăn',m)+block('Không phép',k);});}).catch(function(){});
})();
