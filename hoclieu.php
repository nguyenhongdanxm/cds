...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_item') {
        ...
        if ($index >= 0) {
            ...
            if (!$isAdmin) { http_response_code(403); exit('Chỉ quản trị được sửa học liệu.'); }
            ...
            $data['items'][] = [
                ...
                'author_id' => $userId, 'author' => $userName, 'approved' => $isAdmin,
                'approved_at' => $isAdmin ? date('c') : '', 'approved_by' => $isAdmin ? $userName : '',
                'featured' => false, 'created_at' => date('c'), 'updated_at' => date('c'),
            ];
        }
        save_json($dataFile, $data);
        flash($isAdmin ? 'Đã lưu và hiển thị nội dung.' : 'Đã gửi nội dung, vui lòng chờ quản trị duyệt.', 'success');
        lh_redirect($tab);
    }

    if ($action === 'approve_item' || $action === 'feature_item' || $action === 'delete_item') {
        if (!$isAdmin) { http_response_code(403); exit('Chỉ quản trị được thực hiện thao tác này.'); }
        $id = trim((string)($_POST['id'] ?? ''));
        $index = lh_find($data['items'], $id);
    ...
    if ($action === 'save_link') {
        if (!$isAdmin) { http_response_code(403); exit('Chỉ quản trị được quản lý liên kết.'); }
        $id = trim((string)($_POST['id'] ?? ''));
        $title = trim((string)($_POST['title'] ?? ''));
...
<li><a class="nav-link <?= $tab==='resources'?'active':'' ?>" href="?tab=resources"><i class="bi bi-collection me-1"></i>Học liệu</a></li>
<li><a class="nav-link <?= $tab==='exams'?'active':'' ?>" href="?tab=exams"><i class="bi bi-ui-checks me-1"></i>Kiểm tra và thi</a></li>
<li><a class="nav-link <?= $tab==='links'?'active':'' ?>" href="?tab=links"><i class="bi bi-link-45deg me-1"></i>Liên kết</a></li>
</ul>
...
<?php endforeach; ?></div>
<div class="d-flex justify-content-between align-items-center mb-3"><h4 class="fw-bold mb-0">Học liệu nổi bật</h4><?php if ($isAdmin): ?><span class="small text-muted">Quản trị đánh dấu nổi bật trong danh sách</span><?php endif; ?></div>
<div class="row g-3 mb-4">
...
<div class="modal fade" id="itemModal" tabindex="-1"><div class="modal-dialog modal-lg"><div class="modal-content"><form method="post" enctype="multipart/form-data" id="itemForm"><div class="modal-header"><h5 class="modal-title">Nhập <?= $isExam?'đề, bài kiểm tra hoặc kỳ thi':'học liệu' ?></h5><button class="btn-close" type="button" data-bs-dismiss="modal"></button></div><div class="modal-body"><input type="hidden" name="csrf" value="<?= e($csrf) ?>"><input type="hidden" name="action" value="save_item"><input type="hidden" name="section" value="<?= $isExam?'exam':'resource' ?>">
